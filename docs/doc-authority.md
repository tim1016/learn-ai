...
| 0047 | Authority recovery (`rebuild_from_mirror`, `reset_authority`) is an offline ceremony against a stopped Clerk, never a panel action: recovery refuses while the execution lease lives and the reconciliation sweep is the lease holder, so the capabilities leave the presented surface and a failed authority posts one `terminal` blocker with a `fix_elsewhere` move to `docs/runbooks/alpaca-sqlite-clerk-recovery-and-cutover.md` |
| 0048 | Episode age policies are declared per reason code on `ReasonPolicy` with a required no-default terminal disposition; `holds` is retired into `uncertainties` at schema v12 (at most two rows, and the hold fold kinds stay registered read-only so pre-v12 mirrors remain replayable); the admission-marker substrate is settled negatively only — the Alpaca SQLite clerk cannot host it, since marker users and clerk users are disjoint sets |
| 0049 | The Polygon→LEAN data lake is the market-data authority for historical bars — immutable content-hashed files canonical, Postgres catalog/coordination only, no bar bytes; scoped compatible with ADR 0001, with an exact rebuildability breakdown (identity/hash/coverage re-derivable from bytes, lease state deliberately not, provenance not — #1840 owes migrating the file-side provenance home into the lake before the policy tree is deleted); records the deliberate deviation from the pruned 2026-05-20 spec's Backend-orchestrated flow to in-process Python orchestration at run materialization, naming what that gives up (partial-coverage policy, `request_id` idempotency) and who now owns it; records the provider-licensing retention/redistribution caveat as an open gate, not assumed passed; defers TimescaleDB to a future reader-side projection pending a live-ingestion or large-universe trigger (live-ingestion is a separate future ADR, since live ticks never enter the lake) |
| 0050 | Supervised execution-lease revival is fenced at the store: the heartbeat revives an expired lease only via an atomic CAS on unchanged owner token + authority generation pinned at open (proving no other writer or ceremony ever held the account, per 0048 D4f), retries transient renewal errors instead of dying, and exits only on store-proven loss where 0047's restart ceremony remains the sole cure; terminal evidence keeps its durable file-side home and the boot scan's repair pass also runs in-process on revival (revival-stamped provenance, same-run terminal evidence preserved) |
| 0051 | Cohort-scoped flatten is N attributed per-bot flatten-class legs behind one affordance, never an account-level position flatten: membership is explicit in the request (the backend never infers it at act time), the presentation GET carries each member's actual presented action facts (ADR 0047), per-leg idempotency identity is derived as {cohort_key}:{sid}, a leg's own refusal never aborts its siblings (only account-scoped authority loss ends the batch early), legs run sequentially on the single-writer account, and no new per-bot action id or vocabulary entry is introduced |
| 0052 | Archive is an operator-declared terminal exit for a bot the operator is finished with, and terminal rows leave the read path: a new action id that leaves #1795's provably-dead Retire contract untouched, gated on custody rather than inadmissibility (so unprovable flatness under an account freeze refuses, where retire's custody check is only a backstop), re-proved at commit against a freshly reconciled snapshot, committing the same STRATEGY_INSTANCE_RETIRED transition so one terminal phase carries both meanings; the catalog then skips the custody projection and lifecycle file read for a retired row one batched query proves inert, and the cohort form follows ADR 0051's explicit-membership shape |
| 0053 | Feed continuity is same-run recovery under the consumer's decision clock, and substitution is fail-closed: a broker-socket interruption is waited out against a `ContinuityPolicy` the bot authors (decision session, calendar-backed trigger clock, substitution authority, evidence sink) rather than ending the run, a `MinuteAssembler` owned by the consumer stitches the open minute across the reconnect so a resubscribe can no longer deliver a damaged mid-bucket minute, completeness is proven by 12-of-12 contribution count in RTH, and every minute an interruption touched that cannot be proven complete is refused inside the decision session and recorded as a `gap` outside it — no historical bar is ever fetched to fill a live gap in this build, and every continuity fact is journalled into the source-bar ledger's single `evidence_seq` order before the bar it explains is delivered |
| 0054 | Corpus coverage is a stamp on a proven paper account and a blocker anywhere else: `parameters_match_validated_settings` is no longer a seal check but a `corpus_coverage` field (COVERED / UNCOVERED / NOT_CHECKED) on a PROVEN build fact, carried into the admitted decision's explanation and the run's schema-3 evidence so an exploratory run is never replayed as citable qualification evidence; `evaluate_run_admission` refuses UNCOVERED as `PROGRAM_CORPUS_UNCOVERED` unless the Clerk custody snapshot positively names `account_mode="paper"` (`None` fails closed like live, the synthetic Dry Run authority is paper by construction, and the canary allowlist still gates first); no configuration flag, and deploy-time qualification and stranded-bot rescue are recorded as not decided |
| 0055 | Research sweep tables (Grid Search cells and searches; Walk-Forward studies) are owned by the Python service, schema and writes alike: idempotent versioned DDL applied on first use under an advisory lock, asyncpg writes on the one named background loop with FastAPI reading through its own loop's pool, every chunk write and terminal transition fenced by an atomically claimed attempt generation, and Grid Search history filtered by an ownership column rather than a caller-set flag; EF Core neither knows nor migrates these tables |
| 0056 | A walk-forward study is a procedure over Grid Search — per fold, a training sweep whose ranking-contract leader is the winner, then a test sweep of the same grid with every non-winner cell marked exploratory — recorded as `research_walk_forward_studies` with the same attempt fence, frozen once (one data snapshot, one code identity, fold sweeps must agree byte-for-byte), judged by the frozen verdict over fold-winner retention (median, `ceil(S/2)` coverage floor, 0.5 threshold, always "based on D of S folds"); the SPY EMA normalized-gap protocol, its Exhaustive Run and their job types, pages and manual are retired, their on-disk artifacts untouched |
| 0057 | The Recency Chart's four tables, their writes (launch creation, snapshot persistence with redelivery answered by cell identity, terminal status, soft-delete / restore) and their reads (overlap for the chart, entry-inside-window for the hero, newest-live-run representative, deleted-owner visibility, JSON numbers on the wire) move to the Python service in one change; the tables are adopted by `schema.py` v3 with `IF NOT EXISTS` and the EF migration that removes the entities is a tool-generated, hand-emptied no-op (snapshot untouched) so no row is dropped; the study hard-delete guard keeps a raw read-only lookup; cutover is a coordinated drain |
| 0058 | Backtest runs (engine, LEAN sidecar and spec-strategy) are persisted, read and compared by the Python service in one change: fresh tables `research_backtest_runs` / `research_backtest_run_trades` / `research_parity_verdicts` (schema v5; rows expendable, no migration), one canonical persist payload through one converter and one write, best-effort persistence preserved, the parity verdict ported in-process, reads under `/api/research/backtest-runs` with the GraphQL-era camelCase field names kept, the Recency hard-delete guard as a join, and the .NET study surface (APIs, resolvers, services, entities, tables) deleted rather than proxied |
| 0059 | Real-money Alpaca Live is reachable through graduation (the live cutover) and a per-instance arming ceremony that lapses on an operator-set count — shadow rehearsal under an isolated `shadow:` authority is optional (amended 2026-09-09) — and a cash-bound risk envelope — the cash bound counts working ENTERs, the daily loss hold is account-wide, and nothing else is restricted (no per-order cap, symbol allowlist or session restriction); extended hours and every symbol are in scope; every envelope value comes from the environment file, is validated within its domain and sealed at arming; a live authority exists only on three-way mode agreement; fees are observed as truth with a dated Alpaca model; supersedes ADR 0042's "Live remains unreachable" and PRD FR-035, amends ADR 0021, extends ADR 0011; D8's halt refuses new entries without pausing and D2 names sealed_account_id (amended 2026-09-09) |
| 0060 | Broker configuration is a user-owned named profile in a dedicated SQLite database on the Clerk volume — not Postgres (ADR 0001 and ADR 0035 §6 stand unchanged), and not inside any account's custody database — owned by a durable local-owner record the server resolves and a request can never claim; staged, effective and sealed are three distinct states and only an explicit Apply moves the middle one, so a crash, an automatic restart or a refused preflight boots the last-effective revision and a changed value reaches live entries and exit pricing only through a re-arm that seals it; one worker per installation with no worker identity in v1 (a Paper twin is its own installation, cross-installation multi-worker deferred); the six envelope values must round-trip to bit-identical Python types so `LiveEnvelopeValues.sha` is unchanged and historical seals still verify; offline Paper developer reset clears the target Paper account's state and associated configuration while preserving Live profiles, other accounts, owner and immutable audit history; supersedes ADR 0059 Decision 4's environment-source-of-truth clause **only**, leaving required-when-live, in-domain validation, no-defaults-in-code and sealed-at-arming exactly as written |
| 0061 | Golden Validation is a human promotion record for one exact strategy/program version, signal ticker, resolved parameters, data/window and execution-assumption configuration: immutable computed engine evidence (`agreement` / `divergence` / `unavailable` / `incomplete` / `corrupt`) remains separate from the reviewer’s immutable accepted classification (`Engine agreement` / `Reviewed deviations` / `Manual override`), so a documented deviation or explicit override can satisfy only the strategy-validation gate for that configuration in Paper or Live without bypassing any other safety gate; a paired Python+LEAN history run is normal, Python-only and historical null `program_version` are conspicuous Manual override cases until exact evidence exists, QuantConnect is supplementary, historical v2 parity verdicts retain their semantics while v3 adds parameter/data/timestamp/synthetic-exit receipts, and golden fixtures / Signal Program corpus qualification / v1 events remain distinct |
| 0062 | A broker-neutral fleet control plane addresses isolated broker clerk agents: a small Python coordinator owns identity, discovery, exact routing, broker-qualified account-assignment fencing, health and routing receipts while execution stays in provider-owned clerk processes — one clerk, one process, one distinct physical volume with a verified identity marker, two fences (local custody lease per volume plus a `(broker, canonical external account)` fleet assignment that never expires into takeover and is reassigned only by a proof-driven host ceremony), opaque backend-issued `clerk_id` / `volume_id` / `worker_key` identities with monotonic routing epochs and binding generations, explicit `{broker}/{clerk_id}` routing with fail-closed refusal families and immutable agent clients, a closed typed capability vocabulary with no cross-provider parity, ADR 0060's Stage→Apply→restart semantics preserved per clerk, and ADR 0059's arming and envelope semantics retained per live clerk; enrollment/retirement/reassignment stay host ceremonies in this delivery (browser-driven provisioning deferred until operator authentication and a scoped host-provisioning agent exist, not forbidden); supersedes ADR 0060 Decision 5's one-worker/no-worker-identity clause **only** and extends ADR 0059's one-live-account-per-installation scope |
| 0063 | Draining is a written lane state and **order-quiet is not observable by the fleet layer today**: routing receipts settle `delivered` on the lane's HTTP response rather than on the order, the 76-operation routed catalog is a bot/configuration control surface so bot-runner orders never enter it, routed SSE streams open no receipt, and the ledger leaks unsettled rows — so the receipts table is *command quiet* (the coordinator lost no outcome), never order quiet, and `_CapacityPool._inflight` fails identically as a per-process in-memory HTTP counter a restart zeroes. A host `drain` ceremony moves `provisioned -> draining` with a durable `draining_since_ms`, ungated on entry because the transition itself closes the router's door, and irreversible; `draining -> retired` requires **lane quiet** — amended 2026-09-19 to five conditions the lane asserts together: draining, no bot running (runner-confirmed, not the panel's desired state), every working order **on the account** ended at the broker (a resting GTC included; "cancel sent" is not an ending), the whole account flat, and no order intent in flight — the last three read from the provider's own order and position lists rather than from bot-attributed custody, so a position or order opened by hand is named rather than silently passed — confirmed by the current session under `confirm_assignment`'s instance/epoch fence — answered by the Alpaca lane on every heartbeat while draining since 2026-09-22, so a lane that proves it retires on the normal path, and one that does not is refused rather than degraded to an attestation; the only relief is a separately named, operator-attributed `force-retire` that settles unsettled attempts `outcome_unknown` and records `lane_confirmation: absent`, never a fallback inside `retire` and never a reaper. Its deadline is two-legged and calendar-derived — `max(draining_since_ms + drain_deadline_ms, the close of the first session opening at or after draining_since_ms)`, all `int64 ms UTC`, with a 24-hour floor on the duration — because a flat wall-clock constant buys five minutes of session when a drain is entered at 15:55 ET and no session at all across a holiday weekend, and reasoning a session boundary out of a duration is what `temporal-rigor.md` bans. `RELEASE_PROOF_TOKEN` is deleted, only the mount attestation survives (bounded attributable `operator`/`change_ref`, attribution not proof, because the host alone sees its own mounts), both `release_assignment` and `reassign_assignment` require a draining, command-quiet predecessor that has also waited out the same deadline that bounds `force-retire` — which makes **reassignment unreachable outright** until #2155 closes, since the same ADR forbids reassigning any drained lane, an unresolved collision the ADR states as a conclusion rather than papering over (release is unaffected, so drain → wait → release → `force-retire` stays a complete exit, and #2151/#2150 record why blocking lane-to-lane reassignment costs little) — and the `provisioned -> retired` bypass is closed both by an enforced three-clause "never served" predicate and by a schema-trigger backstop mirroring it: no row in the append-only assignment history, none in the append-only session history, **and no live `clerk_sessions` row**, because that table is a per-clerk upsert that is never deleted, so a clerk that registered exactly once has a live row and no history row and the two history tables alone would misclassify it as never-served. Before the provider shipped every retirement of a served clerk was a `force-retire`; it now remains the exit only for a lane that cannot answer, and since 2026-09-22 release, re-reservation and reassignment consume the same confirmation (schema v7 making one live assignment per clerk structural), so an account moves to another lane only on its old lane's own quiet proof. Open holes named with blast radius: `ConfirmationEvidence` carries no lifecycle field so ADR 0062's FR-066 offline recovery cannot distinguish *any* drained lane from a healthy one, not only one drained mid-outage; handover carries a two-writer hazard with no successor-side mitigation, and blocking reassignment does not close it because `release_assignment` followed by `reserve_assignment`'s released-key reuse is the same handover in two steps with less evidence; a resting GTC order is a working order and lane quiet is still not scoped around it, but the 2026-09-19 amendment reverses its *permanence* — the operator cancels it on the normal path, on the same reasoning §8 used to accept flatten-before-handover, so it no longer makes its lane force-retire-only; forced-unknown obligations outlive the clerk that owned them; per the owner decisions of 2026-09-15 and 2026-09-19 no drain gate reads a position, predecessor-record adoption is rejected outright with flatten-first as the standing handover rule, and an account holding open positions or working orders is not handed to a successor — enforced as provider-owned nothing-open evidence on both handover paths when #2111's ceremonies ship (§8 amendment) |
| 0064 | Alpaca navigation is **account-first**: one account workspace (account name as switcher, Paper/Live mode, equity, sync indicator, Deploy) over Overview / Bots / Gallery / Configuration tabs, with a bot's page inside under the tab it was opened from; `/brokers/alpaca` is the only multi-account page, and the broker-wide Bots/Gallery choosers and the menu's Deploy item redirect to it; top-bar account badges open their account (same tab when already inside one); switching accounts keeps the tab; one account name (nickname, else lane label; a shared name is shown with its lane label, never refused); the account number only on Configuration and in consequential confirmations (ADR 0059 D8 amended); window titles carry the account. FR-091 kept — nothing remembers a last-used account — and FR-094/095/096 kept: badges and switcher are navigation, never command authority, and a not-ready account's tabs explain themselves in place |
