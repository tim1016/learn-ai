...
| 0049 | The Polygon→LEAN data lake is the market-data authority for historical bars — immutable content-hashed files canonical, Postgres catalog/coordination only, no bar bytes; scoped compatible with ADR 0001; records the deliberate deviation from the pruned 2026-05-20 spec's Backend-orchestrated flow to in-process Python orchestration at run materialization; records the provider-licensing retention caveat as an unresolved gate; defers TimescaleDB to a future reader-side projection pending a live-ingestion or large-universe trigger |
