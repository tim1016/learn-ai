...
| 0049 | The data lake — immutable LEAN-format files canonical, Postgres as catalog and coordination plane — is the authority for historical bar data, scoped as compatible with ADR 0001's files-first doctrine (the catalog's index is rebuildable by re-walking and re-hashing, provenance keeps a file-side home so the audit trail stays on files, and lease state is deliberately not rebuilt); run materialization orchestrates in-process in Python, deliberately not the 2026-05-20 spec's Backend-driven `ensure_data`/`prepare_run` flow; the provider-licensing gate on retention is recorded open, not assumed passed |
