...
| 0059 | Real-money Alpaca Live is reachable only through a shadow gate (every sealed instance shadows the live account under an isolated `shadow:` authority before it may arm), a per-instance arming ceremony that lapses on an operator-set count, and a cash-bound risk envelope — the cash bound counts working ENTERs, the daily loss hold is account-wide, and nothing else is restricted (no per-order cap, symbol allowlist or session restriction); extended hours and every symbol are in scope; every envelope value comes from the environment file, is validated within its domain and sealed at arming; a live authority exists only on three-way mode agreement; fees are observed as truth with a dated Alpaca model; supersedes ADR 0042's "Live remains unreachable" and PRD FR-035, amends ADR 0021, extends ADR 0011; D8's halt refuses new entries without pausing, D2 names sealed_account_id, and D2/D3 no longer require a shadow receipt to arm (amended 2026-09-09) |
