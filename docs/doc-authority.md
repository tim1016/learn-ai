...
| 0063 | Draining is a written lane state and **order-quiet is not observable by the fleet layer today**: routing receipts settle `delivered` on the lane's HTTP response rather than on the order, the 76-operation routed catalog is a bot/configuration control surface so bot-runner orders never enter it, routed SSE streams open no receipt, and the ledger leaks unsettled rows — so the receipts table is *command quiet* (the coordinator lost no outcome), never order quiet, and `_CapacityPool._inflight` fails identically as a per-process in-memory HTTP counter a restart zeroes. A host `drain` ceremony moves `provisioned -> draining` with a durable `draining_since_ms`, ungated on entry because the transition itself closes the router's door, and irreversible; `draining -> retired` requires **lane quiet** — no working order, no admitting runner — confirmed by the current session under `confirm_assignment`'s instance/epoch fence, which no provider can answer yet, so the normal retirement path **ships blocked rather than degrading to an attestation**; the only relief is a separately named, deadline-conditioned, operator-attributed `force-retire` that settles unsettled attempts `outcome_unknown` and records `lane_confirmation: absent`, never a fallback inside `retire` and never a reaper. `RELEASE_PROOF_TOKEN` is deleted, only the mount attestation survives (bounded attributable `operator`/`change_ref`, attribution not proof, because the host alone sees its own mounts), both `release_assignment` and `reassign_assignment` require a draining, command-quiet predecessor that has also waited out the same deadline that bounds `force-retire`, and the `provisioned -> retired` bypass is closed both by an enforced "never served" predicate and by a schema-trigger backstop against the append-only assignment and session history tables. Until a provider ships lane quiet, every retirement of a served clerk is a `force-retire`, not an exception — the auditable count starts at 100%. Open holes named with blast radius: `ConfirmationEvidence` carries no lifecycle field so ADR 0062's FR-066 offline recovery cannot distinguish *any* drained lane from a healthy one, not only one drained mid-outage; release and reassignment carry the same two-writer hazard with no successor-side mitigation; forced-unknown obligations outlive the clerk that owned them; per the owner decision no drain gate reads a position, and adopting positions into a successor's custody remains a provider-owned ceremony that does not exist |
