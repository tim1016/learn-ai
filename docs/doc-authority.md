...
| 0063 | Draining is a written lane state, not just vocabulary: a host `drain` ceremony moves `provisioned -> draining` with a durable `draining_since_ms`, entering it is deliberately ungated because the transition itself is what closes the router's door, and it is irreversible; `draining -> retired` is gated on three observed facts — coordinator quiet (zero routing receipts with `state='not_dispatched' AND dispatched_at_ms IS NOT NULL`, the registry's own durable obligation ledger), lane quiet confirmed by the current session under `confirm_assignment`'s instance/epoch fence (never a heartbeat observation, never `_CapacityPool._inflight`, which is a per-process in-memory HTTP counter that a restart zeroes and that never sees a working order), and no unreleased assignment — so `RELEASE_PROOF_TOKEN` is deleted and only the mount attestation survives as an operator input, recorded as bounded attributable `operator`/`change_ref` because the host alone can see its own mounts; a stalled drain is bounded by an absolute `draining_since_ms + drain_deadline_ms` deadline after which the operator (never a reaper) may force still-in-flight attempts to `outcome_unknown` as a named reconciliation obligation; drain composes with `reassign_assignment` (predecessor must be draining and coordinator-quiet) rather than being absorbed by it; and per the owner decision no drain gate ever reads a position — open positions stay with the account and adopting them into a successor's custody remains a provider-owned ceremony that does not yet exist |
