...
| 2026-05-06 | F-0004 (P1) — `strategy_engine.py::AnalyzeOptionsStrategy` no registry row | Row added to § Options pricing and Greeks | grep `strategy_engine` in registry returns the new row | Stage B commit |
| 2026-05-06 | F-0005 (P1) — `engine/options/pricer.py` undocumented dispatcher | Row added to § Options pricing and Greeks; PricingMode values noted | grep `engine/options/pricer` in registry returns the new row | Stage B commit |
...
| 2026-05-06 | F-0027 partial — provenance blocks for `engine/indicators/` subtree | 4-field block added to `sma.py`, `ema.py`, `rsi.py`, `macd.py`, `adx.py`, `supertrend.py`. (PositionEngine + SnapshotService::ComputeMetrics also got blocks via F-0010/F-0011.) Status remains `deferred` overall; the rest of the canonical-math surface (services + volatility + research) follows the burn-down-on-touch pattern. | grep `Formula:` in `app/engine/indicators/*.py` returns 6 hits | Stage E commit |
| 2026-05-06 | F-0030 partial — 5 internal-only reference notes | Created `docs/references/{portfolio-valuation,portfolio-reconciliation,replay-determinism,trade-divergence,fifo-accounting}.md`. Status remains `deferred`; the harder `(verify)` references and Hull-citation extracts are still owed. | New files in `docs/references/` | Stage E commit |
| 2026-05-06 | F-0027 further partial — 5 canonical math services + statistics | 4-field provenance block added to `bs_greeks.py`, `quantlib_pricer.py`, `volatility/solver.py`, `volatility/vix_replication.py` (cites CBOE VIX 2019 / DDKZ 1999 explicitly per F-0007 high-stakes flag), `engine/results/statistics.py` (Sharpe 1994 + Bacon §8.2). Status remains `deferred`; volatility-subtree non-headline files (fitting, surface, models, basis, etc.) and research subtree (signal/, validation/, edge/) still owed. | grep `Formula:` in those 5 files returns 5 hits | Stage F commit |
