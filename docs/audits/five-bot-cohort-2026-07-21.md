...
- **Recovery-path defects and fixes:** The first UI flatten attempt sent no order because the host-native emergency subprocess inherited the container-only `host.containers.internal` address. After correcting that mapping, it correctly reached the Gateway but inherited colliding client ID `42`; it again sent no order. The host daemon now maps the alias to loopback and assigns emergency flatten a dedicated client ID (`1000000`), both pinned by the focused host-daemon regression test. The daemon was restarted to load the local fix.
