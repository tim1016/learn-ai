...
| **`.claude/rules/python.md:52` names a deleted file** | Found by this receipt; **not fixed** | The "Live-control router freeze" rule reads *"While `app/routers/live_instances.py` exceeds this threshold, a PR may not increase its net physical line count."* PR-B deleted that router. The rule is now unsatisfiable as written and its 1 000-line freeze has no subject. It is a **binding instruction-layer rule**, and deciding whether the freeze should now attach to another router (`routers/broker.py` is 614 lines, `brokers.py` 655 — neither exceeds the threshold) is a policy call, not a documentation edit. Left for the controller. `AGENTS.md:11` **was** updated by PR-B and correctly states the deletion. |
