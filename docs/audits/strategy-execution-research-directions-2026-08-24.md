...
| `flatten_stop` performer (`panel_data_source.py:1016-1053`) | Guard + performer + `flatten_supported=True` exist, but the performer is **unexercisable under SQLite custody** — `require_active_run` (`idempotency.py:77`) refuses EXITs once the run is retired (which crash recovery and the performer's own first STOP step both do), and its `panel-flatten:` decision id trips `reject_colon` (`exit.py:68`). It is also never presented (`sqlite_panel_adapter.py:61` presents only `resume`) | Same F18 dead end, proven doubly unreachable |
...
**Research questions.**
1. What is the safe executor for `SafeFlattenPlan` under Clerk custody — cancel-first, attributed-quantity-exact, idempotent under the existing claim/fence machinery? *(Resolved by the implementation plan, `docs/superpowers/plans/2026-08-24-exposure-lifecycle-closure.md`: a new recovery-catalog `execute_safe_flatten` action. The `flatten_stop` performer was ruled out by code evidence — `require_active_run` refuses EXITs once the run is retired, and its `panel-flatten:` decision id trips `reject_colon` — so it was never exercisable under SQLite custody, not merely unpresented.)*
