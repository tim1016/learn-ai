...
reference: PRD §12.10 + ADR 0007 (bot control ↔ daemon command channel)
...
The bot control page's "STOP queued on `<run_id>`; awaiting ack" banner never
...
but the bot control page keeps showing the stale "queued ... awaiting ack" detail
...
cleanly. Hours later the bot control page still rendered the queued banner. Hard
...
- The optimistic banner is rendered against the `detail` string returned
  ...
  at `Frontend/src/app/components/broker/bot-control/bot-control.component.spec.ts:95`
...
(Stop, then look at the bot control page, see "awaiting ack ... awaiting ack ..."
...
- **Masks future genuine "lost command" cases.** If the bot control page ever
  starts losing queued commands for real (network drop between
  bot control and daemon, daemon crash mid-queue), the banner will look
...
# 2. After at least one full bar processes, click Stop in the bot control page.
# 3. Engine consumes STOP via command channel and exits cleanly.
# 4. Bot Control displays "STOP queued on <run_id>; awaiting ack".
...
# 8. Hard refresh the bot control page (Cmd+R / Ctrl+R) — banner clears.
```

## Suggested resolution (NOT auto-applied)

1. **In the bot control page's command-acknowledgement reconciliation**, when the
...
3. **Test**: bot control component spec that mocks an empty commands list
...
receipt run wind-down phase. After the bot control page Stop succeeded
