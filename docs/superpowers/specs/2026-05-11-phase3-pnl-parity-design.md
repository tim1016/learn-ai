...
> Given QC's captured `qc_orders.json` + `qc_price_history.csv` for the AAPL single-symbol degenerate of QC's "Precomputed ML Predictions" tutorial algorithm, our engine produces a trade log where for **every** trade: `symbol`, `quantity`, `direction`, `order_type` are bit-exact; `fill_price` matches within `atol=$0.01`; `order_fee_amount` matches within `atol=$0.01` (gated on Branch A — see §2.2.3); per-round-trip realized P&L matches within the propagated tolerance `(|entry_qty| + |exit_qty|) × $0.01 + 2 × commission_atol`; and `fill_time` resolves to the same trading date.

**Note on `PNL_DRIFT`.** Realized P&L is computed on both sides from the captured fill data using the same long-only round-trip formula `(exit_price − entry_price) × |shares| − entry_fee − exit_fee`. By the triangle inequality, fill+fee parity within their respective atols algebraically implies P&L parity within the propagated tolerance — so in the happy path `PNL_DRIFT` cannot fire without `FILL_PRICE_DRIFT`, `QUANTITY_MISMATCH`, or `COMMISSION_DRIFT` also firing. The category is kept in the gating set so the assertion is concrete rather than implicit, and so any sign-attribution or aggregation bug in the round-trip pairing surfaces as a divergence rather than passing silently. Capturing QC's own *reported* P&L from `read_backtest_trades` and comparing directly (rather than just our two computations against each other) is a Phase 3.5 extension — the optional `qc_trades.json` fixture is reserved for that future PR.
