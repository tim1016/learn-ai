...
- Unified backtest-run persistence for engine-source runs. **Update 2026-05-19 post-rebase:** PR #296 landed engine-side persistence (`engine_persistence.py`, `spec_strategy_runner.py`) and widened `BacktestRunPersistenceService.cs` to accept `source="engine"`. The "out of scope" note from the original spec draft no longer applies — the backend contract is fixed and the helpers exist. This branch still does not modify them; Task 12 may reuse the GraphQL `compareBacktestRuns` reconciler pattern from PR #296's `test_ema_crossover_lean_vs_spec.py` if it shortens the parity assertion, but otherwise rolls its own (different oracle: LEAN-vs-hand-coded-engine on Polygon fixture, complementary to PR #296's LEAN-vs-spec on the LEAN data dump).
