...
| # | Question | When |
|---|---|---|
| 1 | Fill-model parity: (a) custom `EquityFillModel` overriding `MarketFill` to use `asset.Close` → signal_bar_close; (b) accept LEAN's native next-minute-open and pin Engine Lab parity-target to `next_bar_open`; (c) `MarketOnOpenOrder` at consolidated-bar close. Recommend (a) since the Engine Lab spec already advertises bit-exact parity with `signal_bar_close`, but spike will confirm. | Day 1 of implementation — a 1-day fixture run and inspection of `order-events.json` will settle it |
