...
- **R4 — Phase at the decision instant, half-open sessions.** A decision at exactly the regular close is POST for an extended binding; at the declared close it is CLOSED and refused (`SESSION_CLOSED_AT_DECISION`). Cost if wrong: one refused receipt per day at 20:00 (the last bucket).
- **R5 — Recovery-created reducing orders stay market DAY** (no decision context; the vendor queues them for the next open). Decision-shaped reducing legs are durable in `EXIT_REDUCING_ORDER_CREATED` facts with omit-when-default serialisation. Cost if wrong: a recovery-time reduce outside regular hours waits until the next open, as it does today.
...
- **R8 — Allowances are read on paper too.** Unset → `use_rth=False` trade-mode Start refused (`EXTENDED_HOURS_ALLOWANCE_UNSET`); dry-run and log-only proceed; the Clerk refuses any extended leg it cannot price. No default in code (D4). Cost if wrong: the operator sets two values in `.env`.
