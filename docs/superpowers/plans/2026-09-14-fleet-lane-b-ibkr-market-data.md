...
```
→ first test fails on `"IbkrMarketDataFeed" in authority`; second fails on `IBKR_READONLY` set to `false` present at `Frontend/src/assets/docs/ibkr-setup-guide.md:36,73`.
...
4. `Frontend/src/assets/docs/ibkr-setup-guide.md` — delete the `IBKR_READONLY` set to `false` row (`:73`) and line `:36`; replace the "Read-Only API / Disabled for order-capable paper testing" row with "Enabled — IBKR order actuation is retired (#1583); this connection is read-only evidence and market data".
