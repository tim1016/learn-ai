...
>
> **Last reviewed:** 2026-07-07 (Fresh-run identity-coherence update: Bot Control carries the status symbol into the deploy form as inherited identity evidence. The public deploy endpoint blocks Deploy & start when the inherited symbol disagrees with the new signal stream or declared stock action-plan symbol until the operator explicitly confirms the exact symbol set. Canonical run identity remains the host-daemon ledger.)
