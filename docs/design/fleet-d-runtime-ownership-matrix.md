...
|---|---|---|---|---|
| Identity-bearing storage | Unique coordinator control volume plus coordinator-only non-custody artifacts; existing research/data-lake binds remain coordinator-owned | Unique Paper volume only | Unique Live volume only | Actual Compose sources, destinations, backend markers, and nonsecret host attestations agree; lane sources differ and neither appears in coordinator mounts |
