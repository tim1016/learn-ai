...
A lane whose own confirmation evidence is its drained tombstone (`lifecycle_state: draining`) while the restored registry still records it `provisioned` is refused (`FleetRegistryBackupPredatesDrain`, reason `fleet_registry_recovery_pending`, #2350): the backup predates the drain and cannot know which lane, if any, took the account over. The tombstone is proof the lane is out, never a reason to re-seat it, so the hold stays closed. Run `restore-registry` again with a backup captured after the drain (and after any successor confirmed the account); never rewrite the drained evidence to force the reconciliation. A backup that already records the lane as draining reconciles normally and leaves it draining.
