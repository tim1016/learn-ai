...
>
> **Last reviewed:** 2026-07-01 (Account Truth registry attribution update — account truth now derives bot ownership from the durable account instance registry with attribution/current-risk namespace views; retired-owner live exposure is a distinct blocker. Freshness caching, submit enforcement, Flex import, Client Portal cross-checks, and post-hoc manual adoption remain follow-up slices.).
...
| 2026-07-01 | Codex GPT-5 | Account Truth registry attribution update: `/account-truth` now reads the durable account instance registry instead of the in-process publisher list, exposes `owner_binding_state`, uses retired bindings for terminal attribution, and emits `retired_owner_live_exposure` when a retired bot still owns a live working order or current position. This is read-side correctness only; cached freshness/readiness and submit enforcement remain follow-up slices. |
