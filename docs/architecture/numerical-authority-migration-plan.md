...
**Historical Phase 3 deferral — not a current-state record.** Phase 3 as originally written assumed `runBacktest` becomes a passthrough to `/api/engine/backtest`. Investigation on 2026-04-27 found:
...
2. The `runBacktest` mutation had two consumers: Strategy Lab and `lean-engine` (then proposed as its replacement). `lean-engine` re-imported `ReplayChartComponent`, `ReplayControlsComponent`, `LeanStatisticsComponent` from inside `strategy-lab/`, and called `marketData.runBacktest(...)` itself. It was therefore not self-sufficient to replace Strategy Lab.

The present repository instead keeps Strategy Lab active; `lean-lab` redirects to it. Treat this section as historical migration analysis, not as a claim that Strategy Lab was removed. The engine-ownership map is the current-state authority.
...
| (separate control-plane stream) | Alpaca SQLite Clerk attributed-position and execution-economics authority (ADRs 0035, 0037) | Inkant | **sole selectable authority 2026-08-19; activation-gated** — schema-v8 adds execution-slice/correction provenance, deterministic FIFO ordering, SQLite-only panel/catalog/chart/history projections, external-order review, and decision receipts. Only a valid account/generation/database-bound activation fence selects custody. Missing, `OFF_DUTY`, invalid, conflicting, or failed activation is unavailable; the legacy JSONL authority and numeric projections are deleted. A fresh explicit in-use-account inventory must pass the all-or-nothing activation qualification gate before deployment. |
