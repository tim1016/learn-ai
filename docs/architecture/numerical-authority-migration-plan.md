...
| (separate control-plane stream) | Alpaca SQLite Clerk attributed-position and execution-economics authority (ADR 0035) | Inkant | **implementation review consolidated 2026-08-11; activation-gated** — schema-v8 adds execution-slice/correction provenance, deterministic FIFO ordering, SQLite-only panel/catalog/chart/history projections, external-order review, and decision receipts. The selector uses SQLite only after a valid account/generation/database-bound activation fence. JSONL stays canonical for unactivated accounts; human activation is #1383. |
