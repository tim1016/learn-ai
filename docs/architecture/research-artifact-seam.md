...
|---|---|---|
| 1 | **Heterogeneous on-disk layout retained.** The descriptor parametrises `(subdir, id_pattern, config_filename, result_filename, parent_run_id_extractor)`. Research `runs/` keeps `ledger.json`; the others keep `config.json`. The names are not arbitrary: `app/research/runs/ledger.py` defines the immutable, hash-addressed `RunLedger`, while the other phases persist mutable inputs as configs. Collapsing would erase a semantic distinction the research codebase already encodes. | Zero migration cost. No bytes change on disk. The first PR is behavior-preserving consolidation, not a schema migration. |
