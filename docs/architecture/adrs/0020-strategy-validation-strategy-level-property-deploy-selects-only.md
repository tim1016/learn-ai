...
**Amended 2026-07-05 by ADR 0023** — the validation *gate* is now a **human flag over Python-vs-QuantConnect engine-match evidence**, not an automatic passing-reconciliation (§1); the Strategy Validation page **runs the engines** and records the flag + evidence snapshot + a required reason rather than only browsing a seeded manifest (§3–§4); the deploy **signal stream now defaults to the validated signal (still freely overridable)**, relaxing §2's "does not default"; and the single **Deploy page re-homes from `Strategy Lab` to the `Broker` group, sibling to `Bots` / Bot Control** (nav). The frame of this ADR — validation is a strategy-level property proven against a QC reference, and Deploy only *selects* — otherwise stands. See `docs/architecture/strategy-validation-deploy-rehome-prd.md`.
