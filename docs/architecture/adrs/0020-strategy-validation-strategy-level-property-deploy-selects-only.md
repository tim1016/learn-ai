...
**Provenance:** Promoted from `Proposed` to `Accepted` on 2026-08-18 under [ADR 0039](0039-adr-status-is-decision-standing.md) Decision 1 — a read-through confirming the decision still stands. No code conformance was checked, and none is implied. Original Status line: Proposed 2026-07-05. Drafted during the 2026-07-05 `grill-with-docs` session ("I don't see the deploy page in navigation — add it, and revisit the deploy strategy page"); vocabulary in `CONTEXT.md` § "Strategy validation & signal stream (sharpened 2026-07-05)". **Supersedes** the "The Deploy a strategy page owns creating *or selecting* this package" clause of `CONTEXT.md` § "Validated strategy package".

**Amended 2026-07-05 by ADR 0023** — the validation *flag* is now a **human flag over Python-vs-QuantConnect engine-match evidence**, not an automatic passing-reconciliation (§1); the Strategy Validation page **refreshes registered engine-match evidence** and records append-only authenticated flag events + immutable evidence snapshots + a required reason rather than only browsing a seeded manifest (§3–§4); deployability is derived from the latest `validated` flag event whose behavioral-equivalence verdict is `accepted_for_deploy`; the deploy **signal stream now defaults to the validated signal (still freely overridable)**, relaxing §2's "does not default"; and the single **Deploy page re-homes from `Strategy Lab` to the `Broker` group, sibling to `Bots` / Bot Control** (nav). The frame of this ADR — validation is a strategy-level property proven against a QC reference, and Deploy only *selects* — otherwise stands. See `docs/architecture/strategy-validation-deploy-rehome-prd.md`.
...
### 4. A Python-owned validated-strategy manifest is the source of truth — seeded, not re-derived

The `strategy → {settings-file ref/sha, QC backtest ID, audit-copy ref/sha, reconciliation-verdict ref, validation-case symbol}` binding is persisted as a **Python-owned validated-strategy manifest**, seeded from the data that already exists (run-ledger bindings, qc-shadow attribution, reconciliation reports). It is **not** re-derived live from `run_ledger.json` — a run ledger is an *execution* artifact, and a strategy must be validatable without ever having been deployed. Auto-population on the deploy form reads this manifest; nothing is re-typed.

### 5. Validation is the gate to deployability

The Strategy Validation catalog lists **all** strategies with a validation state. As amended by ADR 0023, the Deploy flow selects only the current deployable projection: the latest non-superseded flag event must be `validated` and its behavioral-equivalence verdict must be `accepted_for_deploy`. A human-recorded `validated` flag with `evidence_only` or `rejected` evidence remains visible and auditable on the Validation page, but does not appear in the Deploy dropdown.
