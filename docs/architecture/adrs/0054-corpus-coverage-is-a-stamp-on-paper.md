...
The copy for both warnings a PROVEN build can carry lives once, in `app/schemas/run_admission.py` (`WIRING_DRIFT_NEXT_STEP`, `CORPUS_UNCOVERED_EXPLANATION`, `CORPUS_UNCOVERED_NEXT_STEP`), and the live proof and the frozen-run replay compose explanation and remedy through one helper (`proven_build_copy`, beside the copy it composes), so a Start decision and that run's panel afterwards say the same thing. A stamp on a fact that is not PROVEN is refused by the model validator: a stamp qualifies a proof, and without one it would read as a verdict about bytes that were never proven.
...
`ClerkCustodySnapshot.account_mode: paper | live` is the carrier, and it is required — there is no "unknown". The real SQLite Clerk is constructed with the broker account it activated against (the same `read.get_account()` that already refuses a non-paper account), and the synthetic Dry Run authority's one construction site states `paper`; the facade refuses a synthetic authority that calls itself live. A custody producer that has no environment to state cannot build a snapshot at all, so the relaxation can only ever be unlocked by a positive fact, never by the absence of one, and never by the shape of an account id.
