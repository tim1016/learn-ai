...
5. **Shadow synthesizes extended-hours fills honestly.** Outside the regular session the shadow port's `fill_model` is `limit_touch`: filled at the limit only if a bar **after** the decision bar reaches it — the decision bar's own range predates the order and can never fill it — otherwise left working and cancelled where the vendor would cancel. Explicit, per ADR 0002's third invariant.
