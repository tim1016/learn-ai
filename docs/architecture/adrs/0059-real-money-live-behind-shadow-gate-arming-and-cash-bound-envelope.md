...
**Vocabulary:** `CONTEXT.md` § "Live account, shadow, and risk envelope (sharpened 2026-09-07)".
...
5. **Shadow synthesizes extended-hours fills honestly.** Outside the regular session the shadow port's `fill_model` is `limit_touch`: filled at the limit only if the decision bar's range reaches it, otherwise left working and cancelled where the vendor would cancel. Explicit, per ADR 0002's third invariant.
