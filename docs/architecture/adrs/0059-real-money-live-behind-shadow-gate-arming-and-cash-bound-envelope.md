...
*Amended 2026-09-10 (owner decision, same date).* "The envelope uses the sealed values" is the whole rule and not only an ENTER-time equality gate, so read it against every judgement the values drive. The implementation had narrowed it: the loss limit — the one the sync raises the hold on **and** the one the guarded clear refuses against — was computed from the *configured* values, and the extended-hours anchors of Decision 5.3 were captured from the environment when the authority was composed. That let an operator raise `ALPACA_LIVE_LOSS_USD`, restart, and clear a standing loss hold without re-arming (ENTER stayed refused under `LIVE_ENVELOPE_DISAGREEMENT`, so the account was left holdless *and* unable to trade, and the re-arm restored no hold). Both now read the newest arming record's sealed envelope, falling back to the configured values only where no arming record exists at all. A future browser UI may *stage* an edit to these values; a staged edit takes effect on entries and exits at the CLI re-arm and never before. Two consequences follow from making the sealed values load-bearing. First, they carry the same domains the environment does — an arming record whose sealed `loss_usd` is infinite, or whose sealed `xh_exit_bps` is 10 000, is refused as invalid rather than allowed to bound the money. Second, a seal that cannot be *read* is not the same as an account that was never armed: the first is unjudgeable and refuses every ENTER under `LIVE_ENVELOPE_UNOBSERVED`, because falling back to the environment there would let a corrupted ledger relax a limit. The one deliberate asymmetry: an EXIT is never refused or delayed for want of a seal — an unsealed or unreadable ledger prices the exit from the configured allowance, because a hold on the way out would strand a real position.
