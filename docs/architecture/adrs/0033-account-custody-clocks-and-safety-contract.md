...
health; the Clerk argument is the sole account, reconciliation, exposure,
order, hold, freeze, and unresolved-effect authority. No account DTO is a
third policy input, and Angular renders the backend decision rather than
recreating its gates.

All three observations used by Start—process, market data, and Clerk custody—
must be no more than 5,000 milliseconds old at evaluation. A fact exactly at
the boundary remains eligible; a fact one millisecond older blocks with typed
stale evidence. Future-dated facts, unknown process state, non-flat or unknown
custody, unresolved effects, holds, freezes, or unready market data all fail
closed.
...
exact execution-time admission decision.
