...
This document is the standing record of every decision above — D1–D10 and the implementation rulings — which is what D9 asked for and one item wider, since the rulings post-date the spec. The spec remains the full *design* record: the review rounds, the measurements, the predicate table, the authorization producer and the testing plan. A widening of any decision here is argued in the spec first.
...
**What the floor delivers before any authorization exists.** Measured against the 2026-09-02 storm: four of the five blips touched only 12/12 minutes and would have been survived on the same run. The 15:01:33 blip lost one 5-second bar, so it would still have ended the three runs — but at the 15:02 rollover, with a typed refusal naming the unprovable minute, instead of `FEED_DEATH` on a healthy socket 100 ms after a teardown. The nightly 1100 reset and the 00:45 gateway logoff fall outside an RTH decision session, so they become recorded gaps rather than kills. And for a run that carries a continuity policy, the damaged mid-bucket minute of §1.3 is eliminated from the ledger from day one, whatever the interruption cause — the assembler is the part of this that improves *decision inputs*, not just survival.

That last improvement is scoped, not fleet-wide. The surviving `MinuteAssembler` is owned by the policy-carrying path; `_stream_bars_legacy` (`app/marketdata/ibkr_feed.py`) calls `stream_minute_bars` without one, and `bars.py` then constructs a fresh assembler per call, so a stall replacement on that path still rebuilds the current minute from only its post-replacement contributions. The populations this excludes are exactly the ones §8, §12 and §9 name — an unsealed binding, a `use_rth=False` binding, and any run while the kill switch is off — which keep pre-#1921 behavior in full, damaged minute included.
