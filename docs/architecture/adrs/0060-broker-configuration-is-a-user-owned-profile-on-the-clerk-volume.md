...
These are not gaps in the plan — they are choices D1–D5 did not make, and each is recorded rather than guessed. **Two of the five are now resolved (1 and 2); questions 3, 4 and 5 still gate acceptance,** which is why this ADR remains Proposed.
...
1. ~~**A retired user setting left in the environment after cutover.**~~ **Resolved by the owner, 2026-09-10.** Neither of the two options offered: the owner chose the stronger one — **refuse, do not ignore quietly**. Built as Package F.
   ...
   Implemented as a **binding refusal, not a process crash**, and the distinction is deliberate. Decision 7 above forbids a configuration problem from crash-looping the boot (#2014), and the plan's own invariant is that an exit is never blocked for want of a configuration fact. So `resolve_worker_binding` and `cli_binding.effective_broker` return the contract-shaped refusal `retired_environment_settings`: no broker is bound, the gate is closed, the reason names the exact variables to delete, and the service still boots. Removing the lines and restarting is the whole remedy. A crash would have taken the data lake, the backtester and every non-Alpaca surface down for a stale line in `.env`, and would have removed the operator's ability to EXIT an open position — which is the outcome this plan exists to prevent.

   The refusal fires **only once the installation has cut over** — precisely, once `decide()` resolves a saved revision to bind. Before that the same seven variables are the only description of the worker there is, and refusing on them would break every deployment that has not yet run the import.
