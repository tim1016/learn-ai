...
This is only true **after** this ADR is implemented. Today, with no `live_config.sizing` field and the allow-list rejecting it, the 1-share switch *would* be an algorithm edit — which is exactly the world the position-sizing research handoff § 3 (lines 148–152, pruned 2026-07-04; git history) describes. **This ADR supersedes that paragraph**: sizing is decoupled from the algorithm, so the canary fix no longer requires re-cutting the QC anchor.
...
- **`capital sleeve` ≠ `allocation`.** The Python live buying-power budget is a **capital sleeve**; the `.NET`/Postgres `StrategyAllocation.CapitalAllocated` was an after-the-fact attribution record. The two words must not be conflated across stacks. *Amended 2026-09-06 (#1964): the `StrategyAllocation` record and the trade-attribution feature that wrote it were deleted as dead code; the vocabulary rule stands, and a future capital-sleeve layer does not build on that model.*
...
- **Sizing card** — a **new dedicated card** on the bot control panel, next to the provenance card, with three sections:
