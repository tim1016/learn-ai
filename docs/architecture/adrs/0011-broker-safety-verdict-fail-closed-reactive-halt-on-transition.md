...
The `reconnect_recovery` template (`broker_activity_templates.py`) requires `quantity`, `symbol`, `price`, and `order_type` on every row. The `executions_for_reconnect_recovery` adapter populates all four from the `Fill` directly (symbol, quantity, price) and from the `ib.trades()` cache cross-reference (order_type, defaulting to `MKT` only when the Trade is gone). The publisher does NOT author a row that would render a placeholder string; if the adapter cannot produce a truthful `IbkrOrderEvent` for a `Fill` (degenerate Execution, missing symbol) it skips that Fill — surfaced via the publisher's per-event logger, never silently dropped.
