...
**Amended by [ADR 0059](0059-real-money-live-behind-shadow-gate-arming-and-cash-bound-envelope.md) (2026-09-07, Accepted):** §6 guardrail 2 reads "`UNSAFE`/live identity is a hard block"; on the Alpaca V2 path an unarmed live identity remains a hard block and an *armed* instance is admitted under ADR 0059's envelope. The non-consequence "Does not touch real-money/live trading" no longer holds for that path. The IBKR deploy-form decisions here are unchanged.
**Provenance:** Promoted from `Proposed` to `Accepted` on 2026-08-18 under [ADR 0039](0039-adr-status-is-decision-standing.md) Decision 1 — a read-through confirming the decision still stands. No code conformance was checked, and none is implied. Original Status line: Proposed 2026-07-05. Drafted during the 2026-07-05 `grill-with-docs` session (deploy-page redesign); vocabulary in `CONTEXT.md` § "Strategy validation & signal stream (sharpened 2026-07-05)" → "Launch-default posture (deploy)". Inverts the launch default that `Frontend/src/app/components/broker/broker-deploy-form/` ships today.
