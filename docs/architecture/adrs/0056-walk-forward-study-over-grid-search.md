...
3. **The study freezes once.** One data snapshot over the whole range and one code identity at launch; each fold sweep's snapshot must agree byte-for-byte or the fold fails.
