...
- The user-facing path is Strategy Lab's LEAN mode at `/strategy-lab`; `/lean-lab` is a compatibility redirect. There is no user-facing CLI.
...
- **Phase 4 (historical Frontend LEAN Lab)** - was the first user-facing path. Phase 4a shipped the trusted-sample form, 4b the equity chart, **4c the custom-algorithm textarea** (server-side accept of `algorithm_source` on `POST /lean/runs/start`), **4d the run-history sidebar** (`GET /api/lean-sidecar/runs` + sidebar component), and **4e form rehydration on sidebar click** (`getManifest` repopulated symbol/window/cash). The former `/lean-lab` page was retired into a redirect during the Engine Lab overhaul. Strategy Lab now owns the browser workflow: it loads a selected strategy's registered source without probing the launcher, permits an ephemeral edit only for an explicit LEAN run, and sends that source through the existing sandboxed `algorithm_source` contract. Developer CLI helpers may exist for tests or spikes only; they are never the product workflow.
