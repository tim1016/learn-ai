...
1. **Header strip** — bot identity (monospace slug + ticker) on the left; three operator-facing posture pills (**Broker proof**, **Submit**, **Exposure**) rendered as human labels; a right-aligned **broker connection** pill (ticker · reachable/unreachable + last-contact).
...
6. As an operator, I want a **broker connection** pill (ticker · reachable/unreachable, with last-contact age), so that I can tell whether the broker session is up independently of the safety verdict.
...
20. As an operator, I want a **slim degraded banner** when the control plane is unhealthy, showing state and last-contact age with a link to broker status, so that I know actions may apply to durable desired state only.
