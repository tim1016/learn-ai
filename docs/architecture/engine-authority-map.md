...
| Configurable strategy spec (declarative entry/exit/manage rules over indicators) | **Strategy Spec layer** | `PythonDataService/app/engine/strategy/spec/` (`schema.py` Pydantic models, `evaluator.py::SpecAlgorithm`, `primitives.py`, `indicators.py`); canonical fixtures at `app/engine/strategy/spec/fixtures/*.spec.json`; HTTP entry point `app/routers/spec_strategy.py` exposing `POST /api/spec-strategy/backtest`, `GET /api/spec-strategy/schema`, `GET /api/spec-strategy/fixtures`. | Phase 1 (shipped 2026-05-04): equity-only, single-symbol, indicator-driven entry/exit. Three parity-pinned secondary implementations (SPY EMA / SMA / RSI mean reversion) reproduce hand-coded twins trade-by-trade. **Phase 2.1 (shipped 2026-05-04):** ADX / MACD / Supertrend indicators wired through registry; `PnLPercent` / `PnLPoints` primitives; `survival.CLOSE_ALL` manage-layer action with first-match-wins semantics. **Phase 2.2 (shipped 2026-05-04):** `DrawdownFromPeak` (trailing stop) + `BarProperty` (range filter) primitives. **Phase 3a (shipped 2026-05-04):** FastAPI router exposing the spec layer over HTTP — schema export, fixture listing, inline-spec backtest. Schema still admits options-template shapes; the evaluator refuses to run them. | parity-pinned secondary — hand-coded `algorithms/*.py` remain math-authority per `docs/math-sources-of-truth.md`; spec layer drift = test failure. Replaces `app/services/rule_based_backtest.py` per migration **Phase 4** |
