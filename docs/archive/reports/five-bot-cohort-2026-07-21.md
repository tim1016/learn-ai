...
> **Archived because:** This cohort acceptance record is historical evidence; the cohort launcher is retired.
...
### 12:15–12:35 — UI-authorized staggered retry and evidence failure

- **Authorization and outcomes:** The UI again reported zero hard blockers across the same five eligible bots, applied its five-bot stagger preset, and recorded durable receipt `paper-validation-1784654129416-51dcf1fd55d4` at 12:15:29 CDT. AAPL, MSFT2, NVDA, QQQ, and SPY were each accepted through the server-owned stagger schedule; the final acceptance was recorded at 12:35:38 CDT. No duplicate start request was sent.
- **Failure evidence:** The final cohort verdict was `Failed`, with `COHORT_SAMPLE_LATE_OR_MISSING`, zero healthy-overlap seconds, and three samples. AAPL, MSFT2, QQQ, and SPY reported healthy runtime counters; NVDA reported `RUNTIME_NOT_LIVE_OR_FRESH`. The receipt therefore cannot mint a validation certificate and is not a successful five-bot validation.
- **UI freshness correction:** The cohort monitor's Refresh control initially kept rendering QQQ as pending even after its durable 12:30:45 acceptance. The read now includes a cache-busting query parameter, with a regression test, so the displayed Refresh action fetches current server-authored receipt evidence.

### 12:36–12:47 — authorized UI recovery of residual paper exposure

- **Initial safety response:** Account Desk detected subsequent broker executions and marked the account proof stale. The account was reconciled through its declared UI control. When the proof found unresolved paper exposure, the UI declared the account-wide Emergency paper account flatten route and required the explicit `FLATTEN` confirmation.
- **Historical recovery-path defects (superseded):** At the time, the first UI flatten attempt sent no order because the host-native emergency subprocess inherited the container-only `host.containers.internal` address. A temporary dedicated-client-ID workaround followed. This is retained as audit history only: Account Clerk slice 6 removed the host-native emergency subprocess and dedicated emergency client identity. The current Clerk-owned path uses the accepting Clerk's existing fenced paper session; there is no direct CLI/run-scoped fallback.
- **Successful UI action and proof:** From MSFT2 Operations, the UI again declared and confirmed Emergency account flatten. The host audit recorded a successful MSFT sell at 12:46:24 CDT after cancelling zero open orders. Account Desk's attested 12:47:05 snapshot then reported `Positions (0)`, zero initial/maintenance margin, and no open holdings. A server-declared reconciliation was `Clean`/`Flat`; the UI accepted the associated account-freeze clear receipt `acct-recovery-DUM284968-1784656064263-f015df02-bf7e3992` with gate `Pass`.
- **Safety boundary:** Every account mutation was confirmed in the UI. No direct HTTP mutation or CLI flatten was used. The final account proof is clean and flat, but the failed cohort receipt remains failed; none of the affected bots was restarted or treated as validated.
