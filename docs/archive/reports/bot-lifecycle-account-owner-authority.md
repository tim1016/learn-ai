> **Status:** Archived historical record (2026-07-22).
...
> **Archived because:** The retained R2 AccountOwner interpretation has been replaced as the implementation snapshot by the Account Clerk authority.
...
| Low-level broker write | AccountOwner grants the broker write context, the IBKR adapter rechecks the current generation for configured production brokers, paper safety checks run, `order_ref` is required, the broker-order helper requires the grant, contract is qualified, the helper rechecks the current generation again, then `client.ib.placeOrder(...)` is called. | `engine/live/account_owner.py`, `engine/live/account_owner_fence.py`, `engine/live/live_portfolio.py`, `broker/ibkr/orders.py::place_paper_order` |
| Duty and operator control | The lifecycle evaluator is the sole durable writer of duty phase, roster membership, and desired state. Routers and CLI submit commands; Start consults account admission and never clears the `STOPPED` latch. | `engine/live/bot_lifecycle_evaluator.py`, `services/operator_capability.py`, `routers/live_instances.py` |
| Daemon actuation | The daemon supervises processes and reports observed facts/outcomes to the lifecycle evaluator; it does not decide or write duty phase. | `engine/live/host_daemon.py`, `engine/live/bot_lifecycle_evaluator.py` |
...
| `accounts/<account_id>/account_observation_lease.json` | account | Durable, immediately revocable observation proof derived from Account Truth. During shadow mode it records the candidate submit authority without replacing the cached Account Truth gate. |
...
4. AccountOwner validates owner generation at intent acceptance and again immediately after `account_owner_submit_prepared` before issuing the broker write grant. The current-generation provider re-reads `accounts/<account_id>/owner_generation.json`, while the intent and grant keep the writer's acquired generation, so a paused old process is fenced after another process advances the persisted generation.
5. The production IBKR adapter can be configured to reject `place_order` and `cancel_open_orders` unless an AccountOwner write grant is present and its generation still matches the current-generation provider.
6. Managed recovery flatten receives AccountOwner's broker-write runner from `cmd_start`, so its cancel and liquidation writes run under the same grant requirement when a production AccountOwner exists. The operator-confirmed `emergency-flatten` one-shot path also opens a scoped AccountOwner grant for its cancel/liquidation calls and rechecks the account generation artifact when present.
7. `broker/ibkr/orders.py::place_paper_order` and `cancel_paper_order` require an active AccountOwner write grant and recheck the current generation immediately before `IB.placeOrder` / `IB.cancelOrder`. Raw `/api/broker/orders` submit/cancel calls have no browser-held grant and return 403 before `IB.placeOrder` or `IB.cancelOrder`.
8. Shadow/fake direct-submit mode may write `PENDING_INTENT` before `broker.place_order`, then write `SUBMITTED`, `ACK_FAILED_UNCERTAIN`, `SUBMITTED_RECOVERED`, `INTENT_NOT_ACCEPTED`, or `SUBMIT_UNCERTAIN_HALTED`.
9. `place_paper_order` requires `spec.order_ref` and enforces paper safety before `IB.placeOrder`.

Current limitation: submit serialization is still inside each runner process, not a long-lived AccountOwner daemon with IPC intake. The fencing token itself is now persisted and cross-process: every owner acquisition advances `owner_generation.json` under the account-owner generation file lock, and stale writers re-read that file before broker write. The final R3 daemon remains the target for a separately-owned broker session and out-of-process intake orchestration.
...
On runner startup, `run.py` initializes the V1 `AccountOwner` from the persisted `owner_generation.json` record when one exists. If the persisted phase is `accepting`, or if no record exists yet, startup acquires ownership by writing `persisted_generation + 1` to `owner_generation.json` and appends `account_owner_generation_recorded`. If the persisted phase is non-accepting (`reconnecting`, `draining`, or `frozen`), startup preserves that phase and does not rewrite it as accepting. The operator surface may therefore continue to treat missing generation evidence as `unknown`; the accepting proof must be emitted by the runner, not inferred by a status read.
...
| Broker submit safety | `orders.py::_enforce_paper_safety`, reconnect recovery halt, required `order_ref`, low-level AccountOwner grant requirement, `LivePortfolio.account_freeze_provider`, `LivePortfolio.account_registry_gate_provider`, AccountOwner intake/pre-write generation fences, and IBKR adapter AccountOwner write-grant enforcement configured by production runner startup. | Generation fencing is cross-process via `owner_generation.json`; submit serialization remains process-local until the long-lived AccountOwner daemon/IPC process ships. |
...
| AccountOwner generation/fencing token | Shipped in `engine/live/account_artifacts.py`, `engine/live/account_owner.py`, `engine/live/account_owner_fence.py`, `engine/live/live_portfolio.py`, `broker/ibkr/orders.py`, and runner startup wiring in `engine/live/run.py`. Generation is advanced on owner acquisition and checked from persisted account evidence at intent intake and immediately before the broker-write boundary. |
...
The Overview chart's node/edge meaning remains backend-authored through `lifecycle_chart`; Angular owns only fixed graph geometry, node layout, and connector rendering. `PythonDataService/app/services/bot_lifecycle_chart.py` authors optional `source_handle` / `target_handle` values on `LifecycleChartEdge` rows where branch topology matters. `Frontend/src/app/components/broker/bot-control/overview-tab/overview-tab.component.ts` renders those backend-authored edges with the cockpit's HTML/CSS chart; edges without backend-authored handles still render as ordinary source-to-target connectors. This changes no lifecycle authority: Python authors topology, status, labels, and receipts; Angular renders the named handles. The fan-out guard is pinned in Python chart tests and `overview-tab.component.spec.ts` by asserting branch edges use distinct backend-authored handles.
...
`LifecycleNodeCardComponent` renders receipt rows inside the lifecycle node that owns them. This is still display evidence only. The rows do not create a new event log, do not make Postgres canonical, do not prove R3 daemon/IPC single-writer authority, and do not let Angular infer a lifecycle or submit-safety verdict. Angular may render and format these rows; Python remains the author of node status, evidence, receipt labels, and receipt values.

### Stream-Primary Bot Cockpit Snapshot

The Bot Cockpit now treats the Bot event stream as the persistent run-history surface. `BotControlSidePanelComponent` binds the stream to the live run when present, otherwise the evidence run, and renders an honest no-run state with a Fresh run navigation affordance when no run id exists.

`BotEventStreamComponent` renders rows as a vertical feed. At rest each row shows timestamp, severity, event/source labels, headline, narrative, and gate count; expansion shows the backend-authored gate walk, identity, terminal error, and forensic facts. The side panel has no horizontal table layout and no node-inspector pane. Node-scoped evidence lives inside lifecycle nodes, while the lower documentation section renders trader-guidance proof lines, account-owner facts, change-for-next-run prose, and technical diagnostics without command buttons.

Inline stream actions are a frontend join from a closed row-condition map (`BotEventType` plus `TerminalErrorCode`) to the current backend-authored operator action block on `operator_surface.actions` or `host_process.start_capability`. Disabled row actions are native disabled buttons with the server-authored reason copy. Enabled row actions dispatch the same existing Cockpit commands as the verdict-now toolbar; mutation receipt rendering still flows through the Stage 4 `BotSurfaceStore` pending-attempt/SSE path and does not issue a post-action status refetch.

The prior trader-guidance lifecycle timeline pane is deleted from the Cockpit route. The lifecycle projection endpoints and replay/tailer services remain available as rebuildable read-side infrastructure, but the Cockpit no longer fetches `GET /api/lifecycle-projection/timeline` for its run-history surface.
...
The system has two deliberately different proofs. **Observation proof** is a fresh, clean Account Truth assessment: current broker state is fully attributed and may include a non-zero position owned by an active bot. **Recovery proof** is the account reconciliation receipt: observation proof plus resolved exposure/flatness required for freeze clearing and any ADR-required flat start. Recovery proof must not be used to decide whether healthy intraday account observation can continue.
...
`GET /api/accounts/{account_id}/reconciliation/latest` returns the latest receipt without sweeping IBKR. `GET /api/accounts/{account_id}/triage` returns a thin account triage projection over the latest receipt, account freeze evidence, active account registry bindings, account-event evidence (including receipt invalidation and binding history), and the derived invalidation state. Triage is a routing/read model: it authors affected bots and recovery rows, but it does not replace `operator_surface`, run-scoped reconciliation, or Account Truth.
...
| Operator trader guidance and submit-readiness | `PythonDataService/app/schemas/live_runs.py`, `PythonDataService/app/services/operator_surface.py`, `PythonDataService/app/services/operator_trader_guidance.py`, `PythonDataService/app/routers/live_instances.py`, `Frontend/src/app/components/broker/bot-control/overview-tab/trader-guidance-pane.component.*`, `Frontend/src/app/components/broker/bot-control/bot-control-page.component.ts` |
...
| Lifecycle chart node receipts | `PythonDataService/app/schemas/live_runs.py`, `PythonDataService/app/services/bot_lifecycle_chart.py`, `PythonDataService/app/services/bot_lifecycle_receipts.py`, `Frontend/src/app/api/lifecycle-projection.types.ts`, `Frontend/src/app/components/broker/bot-control/overview-tab/lifecycle-node-card.component.*` |
| Bot event stream side panel and row actions | `PythonDataService/app/routers/live_runs.py`, `Frontend/src/app/components/broker/bot-control/bot-control-side-panel.component.*`, `Frontend/src/app/components/broker/bot-control/reused/bot-event-stream/bot-event-stream.component.*`, `Frontend/src/app/components/broker/bot-control/reused/bot-event-stream/bot-event-stream-action.ts` |
| Lifecycle projection read endpoints | `PythonDataService/app/routers/lifecycle_projection.py`, `Frontend/src/app/services/live-runs.service.ts` |
