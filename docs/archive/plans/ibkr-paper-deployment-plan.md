...
> **Archived because:** This phased deployment plan includes retired direct-broker and AccountOwner assumptions.
...
| **8** | **Operational topology** — (T3) Hybrid. Windows host-venv preserved (Gateway + IBC native; bots in host venv; `host_daemon.py` extended to N processes and NSSM-wrapped for boot survival). Podman compose stays observability-only. One Gateway, multiple `clientId`s pinned per strategy spec. The executable live runtime currently streams IBKR `reqRealTimeBars`, so deployable specs must log `bar_source = "ibkr_realtime_bars"` in every `DecisionRow`; a delayed-paper adapter must be implemented before any spec may advertise `ibkr_paper_delayed`. Explicit triggers for (T4) Linux VPS migration are written. | [ADR 0003](architecture/adrs/0003-operational-topology-host-venv.md) |
