...
- **LEAN image (pinned at capture):** `sha256:0b8d4e381b63daaa4cebbea7af294cc5b140793a6fd13f8c9cfd63ef2a2fb24d` — the arm64 derivative committed in PR #466 (`learn-ai/lean-sandbox:arm64-dotnet109`). The manifest's `lean_runtime.container_image_digest` field labels this with the `docker.io/quantconnect/lean@…` prefix because the regenerate script hard-codes that prefix; the digest itself is the derivative's. See "Known mislabel" below.
...
## Known mislabel — `lean_runtime.container_image_digest` prefix

`scripts/regenerate_cross_engine_study.py` line ~361 hard-codes the field as `f"docker.io/quantconnect/lean@{bare_digest}"` regardless of which image was actually run. The arm64 derivative's digest now appears in W12mo cells' manifests under the upstream prefix. The W6mo cells were captured before PR #466 with the upstream image and so their prefix is genuine. Treating the field as authoritative would conflate the two. Fix is a one-line conditional in the regen script that reads which image config produced the digest; deferred to a follow-up so this PR stays scoped to "pin the W12mo slice."
