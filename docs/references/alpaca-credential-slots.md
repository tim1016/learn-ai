...
| Start | Behaviour |
|---|---|
| The worker binds a staged revision an **Apply** named | Refuses the change and boots the last-effective revision. The first Apply has no previous binding. The one-shot request is consumed, so press Apply again after tidying `.env`. |
