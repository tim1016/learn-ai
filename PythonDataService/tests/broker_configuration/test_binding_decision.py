...
def test_the_same_account_under_a_different_revision_still_requires_preflight() -> None:
    """A same-account change still must prove its existing obligations clear."""
    verdict = switch_verdict(
        _candidate(previous_account_id="PA000SAME"), candidate_account_pin="PA000SAME"
    )

    assert verdict is SwitchVerdict.DIFFERENT_REVISION
    assert verdict.requires_prior_account_clear()
