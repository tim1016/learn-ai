...
allowance, so Start and Resume of a regular-hours run refuse
``EXTENDED_HOURS_ALLOWANCE_UNSET`` until the account has one. Live seals the
...
async def test_without_the_pair_the_gate_refuses_and_names_the_profile_field(
    ...
    assert admission.state == "ALLOWANCE_UNSET"
