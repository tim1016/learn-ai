...
async def test_applying_a_new_revision_of_the_same_account_refuses_open_obligations(
    service: BrokerConfigurationService, environment: AlpacaCredentialEnvironment
) -> None:
    """A policy or credential change is a switch even on the same account."""
    ...
    assert isinstance(resolved, BoundWorker)
    assert resolved.context.revision == 1
