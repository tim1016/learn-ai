...
def test_simulate_long_signal_then_time_stop():
    ...
    cfg = TradeSimConfig(time_stop_bars=3, slippage_pct=0.0, commission_per_unit=0.0, spread_bps_stock=0.0)
...
def test_simulate_opposite_signal_exits_position():
    ...
    cfg = TradeSimConfig(time_stop_bars=10, slippage_pct=0.0, commission_per_unit=0.0, spread_bps_stock=0.0)
    res = simulate(bars=bars, signals=signals, config=cfg)
    assert res.stats["n_trades"] == 1
...
def test_simulate_costs_reduce_pnl():
    ...
    cfg_no_cost = TradeSimConfig(time_stop_bars=2, slippage_pct=0.0, commission_per_unit=0.0, spread_bps_stock=0.0)
    cfg_with_cost = TradeSimConfig(
        time_stop_bars=2, slippage_pct=0.001, commission_per_unit=0.005, spread_bps_stock=2.0
    )
...
def test_simulate_cost_attribution_sums_correctly():
    ...
    res = simulate(bars=bars, signals=signals, config=TradeSimConfig(time_stop_bars=2))
    attr = res.cost_attribution
    np.testing.assert_allclose(attr["gross_pnl"] - attr["total_costs"], attr["net_pnl"], atol=1e-9)
