...
- Root generation: `trace_corpus_root(entries)` in
  ...
  and SHA-256.
- Backtest window: `SmaCrossoverAlgorithm.initialize()` sets the window
  itself (`app/engine/strategy/algorithms/sma_crossover.py:124`) —
  `set_start_date(2024, 3, 28)` / `set_end_date(2026, 3, 27)`, identical to
  `ema_crossover_signal`. The generator neither sets nor overrides a
