"""Tests for read-only IBKR order and execution projections."""
...
def _spec(**overrides) -> IbkrOrderSpec:
    # Keep a reference on builder fixtures so read-side evidence projection is
    # exercised with the same identifier shape as historical journal rows.
    base = {
        "symbol": "SPY",
        "sec_type": "STK",
        "action": "BUY",
