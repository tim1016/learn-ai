...
def test_sqlite_catalog_uses_config_identity_and_one_economic_rollup() -> None:
    status = _status(sid=SID, strategy_key="opening_range_breakout", mode="trade").model_copy(
        update={"strategy_label": "Opening Range Breakout — Paper"}
    )
    ...
    assert row.strategy_key != "unknown"
