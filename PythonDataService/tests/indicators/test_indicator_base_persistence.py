...
def test_to_state_dict_includes_common_fields() -> None:
    ...
    ind.update(to_ms_utc(datetime(2026, 5, 18, 14, 0, tzinfo=UTC)), Decimal("100"))
    ind.update(to_ms_utc(datetime(2026, 5, 18, 14, 15, tzinfo=UTC)), Decimal("101"))
...
def test_restore_state_round_trip() -> None:
    ...
    src.update(to_ms_utc(datetime(2026, 5, 18, 14, 15, tzinfo=UTC)), Decimal("101"))
    ...
    assert dst.current_time_ms == src.current_time_ms
    assert dst.previous_value == src.previous_value
    assert dst.previous_time_ms == src.previous_time_ms
...
def test_to_state_dict_decimals_are_strings() -> None:
    ind = _CountingIndicator("X", 3)
    ind.update(to_ms_utc(datetime(2026, 5, 18, 14, 0, tzinfo=UTC)), Decimal("100.123456789012345"))
    state = ind.to_state_dict()
...
def test_update_rejects_non_numeric_timestamp() -> None:
    """Indicator state accepts only canonical int64 ms UTC timestamps."""
    ind = _CountingIndicator("X", 3)
    with pytest.raises(TypeError, match="integer Unix millisecond"):
        ind.update(datetime(2026, 5, 18, 14, 0), Decimal("100"))  # type: ignore[arg-type]
