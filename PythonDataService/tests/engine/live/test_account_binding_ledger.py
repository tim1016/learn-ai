...
def test_binding_write_rejects_a_completed_ledger_decision_without_legacy_projection(
    ...
    def skip_legacy_projection(*_args: object, **_kwargs: object) -> None:
        return None

    monkeypatch.setattr(account_registry, "append_binding_decision", skip_legacy_projection)

    with pytest.raises(RuntimeError, match="without invoking the legacy registry writer"):
        write_account_instance_binding(tmp_path, binding)
