"""Static enforcement that Strategy C's Signal Program artifact digest covers
...
EMA-specific checks (issue #1728 defect 2) for ``spy_strategy_c``'s own
``SignalProgramContract.artifact_paths`` and
``_SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS``. See that module's docstring for
the full defect-2 rationale; this file only re-asserts the same closure
invariant against Strategy C's own roots. The generic closure-walker unit
tests (deferred imports, combined import statements, service-root
resolution) already live there and are not duplicated here.
"""
...
def _spy_strategy_c_signal_contract():
    contract = _STRATEGY_REGISTRY["spy_strategy_c"].signal_program_contract
    assert contract is not None
    return contract


def test_spy_strategy_c_signal_program_artifact_digest_matches_its_signal_decision_closure() -> None:
    contract = _spy_strategy_c_signal_contract()
    digested = set(contract.artifact_paths)

    closure = signal_decision_import_closure(roots=contract.artifact_paths)
    covered = closure - set(_SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS)
    ...
    assert covered == digested, (
        ...
        "decision -> re-qualify) or to _SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS in "
...
def test_exclusion_list_only_names_files_actually_in_the_closure() -> None:
    ...
    stale = set(_SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) - closure
...
def test_exclusions_and_artifact_paths_are_disjoint() -> None:
    ...
    overlap = set(_SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) & set(contract.artifact_paths)
...
def test_every_exclusion_carries_a_non_trivial_reason() -> None:
    for path, reason in _SPY_C_SIGNAL_DECISION_CLOSURE_EXCLUSIONS.items():
