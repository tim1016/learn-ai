"""Static enforcement that the Strategy B Signal Program's artifact digest covers
...
SMA-specific checks (issue #1728 defect 2) for ``spy_strategy_b``'s own
``SignalProgramContract.artifact_paths`` and
``_SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS``. See that module's
docstring for the full defect-2 rationale; this file only re-asserts the same
closure invariant against Strategy B's own roots. The generic closure-walker
unit tests (deferred imports, combined import statements, service-root
resolution) already live in ``test_signal_decision_digest_closure.py`` and
...
def _spy_strategy_b_signal_contract():
    contract = _STRATEGY_REGISTRY["spy_strategy_b"].signal_program_contract
    assert contract is not None
    return contract


def test_spy_strategy_b_signal_program_artifact_digest_matches_its_signal_decision_closure() -> None:
    contract = _spy_strategy_b_signal_contract()
    digested = set(contract.artifact_paths)

    closure = signal_decision_import_closure(roots=contract.artifact_paths)
    covered = closure - set(_SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS)
    ...
    assert covered == digested, (
        ...
        "decision -> re-qualify) or to _SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS "
        "in scripts/run_signal_program_build_qualification.py with a one-line reason "
        "(it provably cannot). "
...
def test_exclusion_list_only_names_files_actually_in_the_closure() -> None:
    ...
    stale = set(_SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) - closure
...
def test_exclusions_and_artifact_paths_are_disjoint() -> None:
    ...
    overlap = set(_SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) & set(contract.artifact_paths)
...
def test_every_exclusion_carries_a_non_trivial_reason() -> None:
    for path, reason in _SPY_STRATEGY_B_SIGNAL_DECISION_CLOSURE_EXCLUSIONS.items():
