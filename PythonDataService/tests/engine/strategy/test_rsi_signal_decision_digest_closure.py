"""Static enforcement that the RSI Signal Program's artifact digest covers
...
EMA-specific checks (issue #1728 defect 2) for ``rsi_mean_reversion``'s own
``SignalProgramContract.artifact_paths`` and
``_RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS``. See that module's docstring for
the full defect-2 rationale; this file only re-asserts the same closure
invariant against RSI's own roots. The generic closure-walker unit tests
...
def _rsi_signal_contract():
    contract = _STRATEGY_REGISTRY["rsi_mean_reversion"].signal_program_contract
    assert contract is not None
    return contract


def test_rsi_signal_program_artifact_digest_matches_its_signal_decision_closure() -> None:
    contract = _rsi_signal_contract()
    digested = set(contract.artifact_paths)

    closure = signal_decision_import_closure(roots=contract.artifact_paths)
    covered = closure - set(_RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS)
    ...
    assert covered == digested, (
        ...
        "decision -> re-qualify) or to _RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS in "
...
def test_exclusion_list_only_names_files_actually_in_the_closure() -> None:
    ...
    stale = set(_RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) - closure
...
def test_exclusions_and_artifact_paths_are_disjoint() -> None:
    ...
    overlap = set(_RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS) & set(contract.artifact_paths)
...
def test_every_exclusion_carries_a_non_trivial_reason() -> None:
    for path, reason in _RSI_SIGNAL_DECISION_CLOSURE_EXCLUSIONS.items():
