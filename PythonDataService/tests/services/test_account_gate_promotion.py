...
def _gate(*, gate_id: str, status: str) -> GateResult:
    return GateResult(
        gate_id=gate_id,
        status=status,  # type: ignore[arg-type]
        source="test",
        operator_reason=f"{gate_id}_{status}",
        operator_next_step=None,
        evidence_at_ms=NOW_MS,
    )


def test_resolve_action_gate_keeps_account_truth_for_a_current_weaker_clerk_proof() -> None:
    truth_gate = _gate(gate_id="account.account_truth", status="block")
    lease_gate = _gate(gate_id="account.observation_lease", status="pass")
    ...
    assert resolution.authority == "account_truth"
    assert resolution.gate == truth_gate
    assert resolution.current_clerk_proof_is_weaker is True


def test_resolve_action_gate_keeps_the_promoted_clerk_gate_when_current_evidence_matches() -> None:
    truth_gate = _gate(gate_id="account.account_truth", status="pass")
    lease_gate = _gate(gate_id="account.observation_lease", status="pass")
    ...
    assert resolution.authority == "observation_lease"
    assert resolution.gate == lease_gate
    assert resolution.current_clerk_proof_is_weaker is False
