...
Both the hand-coded ``RsiMeanReversionAlgorithm`` and the ``StrategySpec``
evaluator are checked against the same oracle, so neither implementation can
drift away from LEAN — or from the other — unnoticed.
"""
...
FIXTURE_DIR = Path(__file__).resolve().parents[2] / "fixtures/golden/strategy-parity/ENG-009/v1"
SPEC_PATH = (
    Path(__file__).resolve().parents[3] / "app/engine/strategy/spec/fixtures/rsi_mean_reversion.spec.json"
)
...
def test_rsi_mean_reversion_matches_lean_fills_and_equity(implementation: str) -> None:
...
def _strategy(implementation: str) -> RsiMeanReversionAlgorithm | SpecAlgorithm:
    if implementation == "hand_coded":
        base: RsiMeanReversionAlgorithm | SpecAlgorithm = RsiMeanReversionAlgorithm(
            symbol="SPY",
            window=14,
            oversold=30.0,
            overbought=70.0,
        ...
        base_class: type = RsiMeanReversionAlgorithm
    else:
        base = SpecAlgorithm(load_spec_from_path(SPEC_PATH))
        base_class = SpecAlgorithm

    class _PinnedWindow(base_class):  # type: ignore[misc,valid-type]
        def initialize(self) -> None:
            super().initialize()
            self.start_date = datetime(2026, 2, 2, tzinfo=_NY)
            self.end_date = datetime(2026, 2, 25, 23, 59, 59, tzinfo=_NY)
