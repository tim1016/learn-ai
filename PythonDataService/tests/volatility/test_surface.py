...
def _bs_price(
    spot: float, strike: float, ttm: float, rate: float, vol: float, is_call: bool
) -> float:
...
class TestSurfaceBuilder:
    ...
    def test_recovers_flat_vol(
        self, flat_vol_chain: list[dict], spot: float, rate: float, base_vol: float
    ) -> None:
        ...
        for k in [90, 95, 100, 105, 110]:
            for t in [30 / 365, 90 / 365, 180 / 365]:
                ...
                assert abs(iv - base_vol) < 0.01, (
                    f"K={k} T={t:.3f}: expected {base_vol}, got {iv:.4f}"
                )

    def test_skewed_chain_builds(
        self, skewed_chain: list[dict], spot: float, rate: float
    ) -> None:
    ...
    def test_diagnostics_populated(
        self, flat_vol_chain: list[dict], spot: float, rate: float
    ) -> None:
...
class TestSurfaceQuery:
    ...
    def test_interpolation_between_expiries(self, surface: VolSurface) -> None:
        ...
        iv_lo = surface.volatility(100.0, ttms[0])
        iv_hi = surface.volatility(100.0, ttms[1])
...
class TestDeterminism:
    ...
    def test_same_inputs_same_surface(
        self, skewed_chain: list[dict], spot: float, rate: float
    ) -> None:
        ...
        for k in [90, 95, 100, 105, 110]:
            for t in [30 / 365, 90 / 365]:
                ...
                assert abs(v1 - v2) < 1e-10, (
                    f"Non-deterministic at K={k} T={t}: {v1} vs {v2}"
                )
    ...
    def test_known_atm_vol(self, spot: float, rate: float, base_vol: float) -> None:
        ...
        for m in np.linspace(0.85, 1.15, 15):
            ...
            if price < 0.01:
                continue
            records.append(
                {"strike": float(k), "ttm": ttm, "option_price": price, "is_call": is_call}
            )
