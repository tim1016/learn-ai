    ...
    *,
    ...
    if chosen.intent is BindingIntent.APPLY:
        if stale_refusal is not None:
            logger.error(
                "Retired broker settings are still present; the Apply is refused",
                extra={
            ...
            recorded = _record_refusal(
                service, candidate=chosen, reason=stale_refusal.next_step
            )
            if chosen.previous_profile_id is None:
                return UnboundWorker(stale_refusal)
            return await _boot_last_effective_after_refusal(
                service=service,
                candidate=chosen,
                reason=stale_refusal.next_step,
                recorded=recorded,
