...
An Apply replacing a different profile or revision must prove the prior account
clear, even if both revisions name that same account. Recovery of the exact
effective revision makes no configuration change and keeps managing exposure.
...
class SwitchVerdict(StrEnum):
    ...
    """A different profile or execution revision on the same account."""
    ...
    def requires_prior_account_clear(self) -> bool:
        ...
        return self in (
            ...
            SwitchVerdict.DIFFERENT_REVISION,
...
def switch_verdict(
    ...
    if candidate_account_pin == candidate.previous_account_id:
        if candidate.replaces_a_different_revision():
            return SwitchVerdict.DIFFERENT_REVISION
