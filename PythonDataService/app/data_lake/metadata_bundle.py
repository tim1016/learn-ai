...
class _MetadataRowClaim(NamedTuple):
    """Result of :func:`_claim_or_reclaim_metadata_row`: exactly one of
    ``artifact_id``/``existing``/``failure_reason`` is non-``None``.
    ``artifact_id`` set means the caller now holds the lease and must
    complete or fail it; ``existing`` set means a concurrent caller already
    published a 'complete' row for this exact digest, nothing to do;
    ``failure_reason`` set means neither a claim nor a reclaim was possible
    right now. ``lease_generation`` (issue #1888) is set whenever
    ``artifact_id`` is, and ``None`` otherwise -- the caller must carry it
    through to ``complete_artifact`` / ``confirm_lease_generation``.
    """
...
async def _claim_or_reclaim_metadata_row(
    *, dch: str, file_path: str, identity: ArtifactIdentity, root_id: UUID
    ...
    if existing is not None:
        return _MetadataRowClaim(None, existing, None, None)

    row_state = await catalog_client.select_metadata_claim_state(dch, data_root_id=root_id)
    if row_state is None:
        return _MetadataRowClaim(None, None, "lease_timeout", None)
    ...
    if current is not None and current.status == "failed":
        return _MetadataRowClaim(None, None, "fetch_timeout", None)
    return _MetadataRowClaim(None, None, "lease_timeout", None)
...
async def _claim_and_complete_metadata_row(
    *, spec: DataRunSpec, kind: MetadataKind, entry: MetadataFileEntry, lake_root: Path, root_id: UUID
    ...
    assert claim.lease_generation is not None, "lease_generation must be set whenever artifact_id is (see _MetadataRowClaim)"
