...
def expected_sessions(start: date, end: date) -> list[date]:
    """Return every NYSE session in ``[start, end]`` (inclusive).

    Inverse of :func:`blocked_dates_in_range`: half-days remain
    sessions, weekends and holidays are omitted. Used by the
    canonical-input fetch to assert no expected session is missing
    from a Polygon response.
    """
...
def session_close_minute_et(d: date) -> int:
    """Return the session-close wall-clock as minutes past midnight ET.

    Regular session closes at 16:00 ET → 960.  Half-days close at
    13:00 ET → 780 (occasionally other times historically, but the
    pandas_market_calendars schedule is the source of truth and is
    used directly here). Raises ``LookupError`` when ``d`` is not a
    session.
    """
    schedule = _CALENDAR.schedule(start_date=d, end_date=d)
    if schedule.empty:
