...
__all__ = ["fold_failed", "fold_order_evidence", "fold_uncertain"]
...
def fold_order_evidence(
    ...
    *,
    ...
    commit; if the process dies between them, whichever hasn't been
    appended yet is what a later recovery sweep must still be able to see
    is missing. A resolution short-circuits on ``orders.broker_order_id``
