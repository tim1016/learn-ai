...
from app.broker.alpaca.clerk.sqlite.projection_errors import (
...
class SqliteClerkProjectionReader:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            # The file can vanish between the ``is_file`` check above and this
            # connect -- a reset running concurrently with a read. Read-only
            # ``mode=ro`` refuses to create it, so sqlite raises here and the
            # app's catch-all would answer 500 on the endpoint whose job is to
            # report exactly this state (#1865 review). Only the
            # missing-database failure is reclassified; anything else is a
            # genuine fault and must keep propagating.
            if not db_path.is_file():
                raise DatabaseMissingAfterEstablishment(
                    f"{db_path} was removed while opening it; this account has no "
                    "activated SQLite authority. Complete the paper cutover before "
                    "reading projections."
                ) from exc
            raise
    ...
    def bot_snapshot(
        ...
        *,
        ...
        operation_limit = projection_helpers.bounded_limit(
            operation_limit,
            maximum=MAX_OPERATION_LIMIT,
        )
        with self._read_transaction():
            self._verify_identity()
    ...
    def operation_timeline(self, effect_operation_id: str) -> tuple[TimelineEntry, ...]:
        ...
        return tuple(projection_helpers.timeline_entry(row) for row in rows)
    ...
    def _runs(self, strategy_instance_id: str | None) -> tuple[ProjectedRun, ...]:
        where, params = projection_helpers.scope_filter("strategy_instance_id", strategy_instance_id)
        rows = self._conn.execute(
    ...
    def _execution_coverage_conflict(
        ...
        proof = execution_coverage_proof(
            self._conn,
        ...
        return ProjectedExecutionCoverageConflict(
            ...
            cumulative_fill_id=None if cumulative is None else cumulative.fill_id,
            exact_qty=None if exact is None else exact.slice_qty,
            exact_price=None if exact is None else exact.slice_price,
            exact_side=None if exact is None else exact.side,
            cumulative_qty=None if cumulative is None else cumulative.quantity,
            cumulative_price=None if cumulative is None else cumulative.price,
            cumulative_side=None if cumulative is None else cumulative.side,
    ...
    def _decode_operation_cursor(
        ...
        *,
        ...
        try:
            ...
            anchor = projection_helpers.operation_cursor_key(payload["anchor"])
            before = projection_helpers.operation_cursor_key(payload["before"])
