...
class SyntheticActivationRecord:
    ...
    @classmethod
    def from_payload(cls, payload: dict[str, Any]) -> SyntheticActivationRecord:
        required = {
            "schema_version",
            "account_id",
        ...
        if not record.db_identity_token or type(record.activated_at_ms) is not int:
...
class SyntheticActivationStore:
    ...
    def append(self, record: SyntheticActivationRecord) -> None:
        ...
        if prior is not None and canonical.authority_generation <= prior.authority_generation:
            raise SyntheticActivationInvalid("synthetic activation generation must increase")
    ...
    def _read_all(self) -> list[SyntheticActivationRecord]:
        ...
        try:
            for raw in self._path.read_text(encoding="utf-8").splitlines():
                if raw:
                    records.append(SyntheticActivationRecord.from_payload(json.loads(raw)))
