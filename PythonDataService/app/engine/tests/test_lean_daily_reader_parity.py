...
from app.engine.data.lean_format import (  # noqa: E402
    PRICE_SCALE,
    LeanDailyDataReader,
)
...
def _read_raw_rows(symbol: str) -> list[tuple[date, int, int, int, int, int]]:
    ...
    if not zip_path.exists():
        raise FileNotFoundError(
            f"Expected LEAN reference daily zip at {zip_path}. "
            f"The parity test needs access to the Lean/Data mount."
        )
    rows: list[tuple[date, int, int, int, int, int]] = []
    with zipfile.ZipFile(zip_path) as zf:
        with zf.open(f"{symbol}.csv") as f:
...
def run_parity_test() -> None:
    ...
    if len(history) != len(raw_rows):
        print(
            f"FAIL: row count mismatch — reader returned {len(history)} "
            f"bars but CSV has {len(raw_rows)} rows"
        )
        sys.exit(1)

    mismatches: list[str] = []
    for bar, raw in zip(history, raw_rows):
        raw_date, raw_o, raw_h, raw_l, raw_c, raw_v = raw
        if bar.time.date() != raw_date:
            mismatches.append(
                f"date: reader={bar.time.date()} raw={raw_date}"
            )
            continue
        ...
        if int(bar.open * PRICE_SCALE) != raw_o:
            mismatches.append(
                f"{raw_date} open: {bar.open} → "
                f"{int(bar.open * PRICE_SCALE)} vs {raw_o}"
            )
        if int(bar.high * PRICE_SCALE) != raw_h:
            mismatches.append(
                f"{raw_date} high: {bar.high} → "
                f"{int(bar.high * PRICE_SCALE)} vs {raw_h}"
            )
        if int(bar.low * PRICE_SCALE) != raw_l:
            mismatches.append(
                f"{raw_date} low: {bar.low} → "
                f"{int(bar.low * PRICE_SCALE)} vs {raw_l}"
            )
        if int(bar.close * PRICE_SCALE) != raw_c:
            mismatches.append(
                f"{raw_date} close: {bar.close} → "
                f"{int(bar.close * PRICE_SCALE)} vs {raw_c}"
            )
        if bar.volume != raw_v:
            mismatches.append(
                f"{raw_date} volume: {bar.volume} vs {raw_v}"
            )
    ...
    # -------------------------------------------------------------------- #
    narrow = list(
        reader.iter_bars(SYMBOL, date(2021, 3, 29), date(2021, 3, 31))
    )
    if len(narrow) != 3:
        print(
            f"FAIL: expected 3 bars in 2021-03-29..2021-03-31, got {len(narrow)}"
        )
        sys.exit(1)
    ...
    # -------------------------------------------------------------------- #
    dst_window = list(
        reader.iter_bars(SYMBOL, date(2021, 3, 12), date(2021, 3, 15))
    )
    if len(dst_window) != 2:  # 03-12 Fri, 03-15 Mon (13/14 are weekend)
        print(
            f"FAIL: expected 2 bars across DST boundary (Fri+Mon), "
            f"got {len(dst_window)}"
        )
        sys.exit(1)
    ...
    if period.total_seconds() != 86400:
        print(
            f"FAIL: daily bar period is {period.total_seconds()}s, expected 86400"
        )
        sys.exit(1)
