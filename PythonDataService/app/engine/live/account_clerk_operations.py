            ...
            async with deps.intake_lock:
                ...
                latest_operation = next(
                    (
                        ...
                        for entry in reversed(deps.journal.snapshot())
                        if entry.emergency_operation is not None
                        and entry.emergency_operation.operation_id == operation_id
                    ),
                    None,
                )
                if latest_operation is None or latest_operation.phase != "bots_paused":
                    raise AccountClerkEmergencyFlattenIncompleteError(
                        "CLERK_EMERGENCY_BOTS_NOT_PAUSED"
                    )
                entries = await asyncio.to_thread(deps.journal.snapshot)
                namespaces = tuple(
                    sorted(
