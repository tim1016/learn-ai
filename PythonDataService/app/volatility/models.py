...
class SurfaceMethodEnum(StrEnum):
    VARIANCE = "variance"
    SABR = "sabr"
    SVI = "svi"
...
class OptionRecord(BaseModel):
    ...
    bid: float | None = Field(None, ge=0, description="Bid price")
    ask: float | None = Field(None, ge=0, description="Ask price")
    open_interest: int | None = Field(None, ge=0)
    volume: int | None = Field(None, ge=0)

    @field_validator("ask")
    @classmethod
    def ask_gte_bid(cls, v: float | None, info) -> float | None:
...
class SurfaceBuildRequest(BaseModel):
    ...
    options: list[OptionRecord] = Field(..., min_length=1, description="Option chain records")
    min_contracts_per_slice: int = Field(5, ge=2, description="Min contracts per expiry to include")
    sabr_beta: float = Field(0.5, ge=0, le=1, description="SABR beta (fixed parameter)")
    build_bid_ask: bool = Field(False, description="Build separate bid/ask surfaces")
...
class SurfaceGridRequest(BaseModel):
    ...
    ttm_list: list[float] | None = Field(None, description="Specific TTMs to evaluate; defaults to fitted expiries")
...
class SurfaceBuildResponse(BaseModel):
    ...
    surface_id: str = Field("", description="Opaque ID for querying this surface later")
...
class ConventionsModel(BaseModel):
    ...
    rate: float = Field(0.05, description="Risk-free rate (continuous)")
    dividend_yield: float = Field(0.0, ge=0, description="Dividend yield (continuous)")
...
class DataFiltersModel(BaseModel):
    ...
    max_spread_pct: float = Field(0.20, ge=0, le=1, description="Max bid-ask spread as % of mid")
...
class BuildFromTickerRequest(BaseModel):
    ...
    method: SurfaceMethodEnum = Field(SurfaceMethodEnum.SVI, description="Surface fitting method")
    conventions: ConventionsModel = Field(default_factory=ConventionsModel, description="Market conventions")
    filters: DataFiltersModel = Field(default_factory=DataFiltersModel, description="Data quality filters")
...
class SurfaceBuildSummary(BaseModel):
    ...
    n_contracts_accepted: int = Field(..., ge=0, description="Number of contracts accepted")
...
class MatrixGridResponse(BaseModel):
    ...
    z: list[list[float | None]] = Field(..., description="IV matrix [n_expiries × n_strikes]")
    x_label: str = Field(..., description="X-axis label (log_moneyness, moneyness, strike)")
...
class MatrixGridRequest(BaseModel):
    ...
    dte_days: list[int] | None = Field(None, description="Specific DTE days to include")
    expiry_dates: list[str] | None = Field(None, description="Specific expiry dates to include")
...
class SmileSliceResponse(BaseModel):
    ...
    fitted: list[SmilePointModel] = Field(..., description="Fitted smile points")
...
class RejectionBreakdown(BaseModel):
    ...
    by_reason: dict[str, int] = Field(..., description="Rejection counts by reason")


class ArbitrageDetail(BaseModel):
    """Arbitrage violation diagnostics."""

    calendar_violations: int = Field(0, ge=0, description="Number of calendar spread violations")
    butterfly_violations: int = Field(0, ge=0, description="Number of butterfly violations")
    severity: str = Field(
        ...,
        description="Violation severity (none, low, moderate, high)",
    )
    worst_slices: list[dict] = Field(default_factory=list, description="Details of worst violations")
...
class DiagnosticsResponse(BaseModel):
    ...
    fitted_params: list[FitParamsResponse] = Field(..., description="Fitted parameters per slice")
    slices: list[SliceDiagnosticsResponse] = Field(..., description="Per-slice diagnostics")
...
class DailySummary(BaseModel):
    ...
    atm_iv: float | None = Field(None, ge=0, description="ATM implied vol")
    rr_25d: float | None = Field(None, description="Risk reversal 25-delta")
    bf_25d: float | None = Field(None, description="Butterfly 25-delta")
    skew_slope: float | None = Field(None, description="Vol skew slope")
...
class BatchSummaryResponse(BaseModel):
    ...
    ticker: str = Field(..., description="Underlying ticker")
    daily_summaries: list[DailySummary] = Field(..., description="Per-day surface summaries")
