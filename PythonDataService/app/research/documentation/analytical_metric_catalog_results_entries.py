...
def _validation_metric(
    ...
    *,
    ...
    trader_interpretation = {
        ...
        "rolling_daily_sharpe": "Compare recent return per unit of volatility with the cumulative dollar path; a falling value can reveal weakening quality before total P&L rolls over.",
        "cumulative_pnl_divergence": "Use the flag as an exploratory decay warning when cumulative P&L trends upward while rolling Sharpe trends downward; do not treat it as a trading signal.",
        "divergence_observation_share": "Use the share to see how often the eligible history showed upward P&L alongside weakening rolling Sharpe, while checking the observation count.",
        "longest_divergence_streak": "Use the longest streak to distinguish isolated divergence noise from a sustained period of weakening risk-adjusted performance.",
    }[metric_id]
    return MetricVariant(
        metric_id=metric_id,
...
PLATFORM_HEADLINE_VARIANTS: tuple[MetricVariant, ...] = (
    ...
    _platform_metric(
        ...
        input_series="Daily returns resampled from the run's marked equity curve, in every mode (compatibility pairs included, owner decision #2424); the closed-trade ledger basis feeds only the engine-parity statistics (``lean_statistics``), never these headline numbers.",
        units="ratio",
    ...
    _platform_metric(
        ...
        category="drawdown",
        input_series="The run's marked equity curve, in every mode (compatibility pairs included, owner decision #2424); the closed-trade ledger basis feeds only the engine-parity statistics (``lean_statistics``), never these headline numbers.",
        units="fraction",
        formatting="percentage, two decimal places",
...
PERFORMANCE_MEMORY_VARIANTS: tuple[MetricVariant, ...] = (
    ...
    _validation_metric(
        "rolling_daily_sharpe",
        "Rolling 20-session Sharpe",
        "Annualized arithmetic mean daily return divided by sample standard deviation over each trailing 20-session native mark-to-market window.",
        input_series="Last native mark-to-market equity observation per America/New_York session, using overlapping trailing windows of 20 returns.",
        units="annualized ratio",
        formatting="two decimal places",
        value_states=(_VALID_ZERO, _PLATFORM_UNAVAILABLE),
    ),
    _validation_metric(
        "cumulative_pnl_divergence",
        "Sharpe–P&L divergence state",
        "True when the trailing 20-observation OLS slope of cumulative native P&L is positive and the matching rolling-Sharpe slope is negative.",
        input_series="Python-authored cumulative native P&L and rolling 20-session Sharpe study points over the same trailing 20-observation trend window.",
        units="boolean diagnostic",
        formatting="Diverging or Aligned",
        value_states=(_PLATFORM_UNAVAILABLE,),
    ),
    _validation_metric(
        "divergence_observation_share",
        "Divergence observation share",
        "Eligible divergence observations flagged true divided by all observations with complete Sharpe and trend windows.",
        input_series="Python-authored divergence flags after both rolling windows are fully observed.",
        units="fraction",
        formatting="percentage, one decimal place",
        value_states=(_VALID_ZERO, _PLATFORM_UNAVAILABLE),
    ),
    _validation_metric(
        "longest_divergence_streak",
        "Longest divergence streak",
        "Maximum consecutive eligible observations whose cumulative-P&L slope is positive while rolling-Sharpe slope is negative.",
        input_series="Chronological Python-authored divergence flags from the saved run.",
        units="sessions",
        formatting="integer sessions",
        value_states=(_VALID_ZERO, _PLATFORM_UNAVAILABLE),
    ),
)
