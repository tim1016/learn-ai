...
class TechnicalAnalysisService:
    ...
    @staticmethod
    def calculate_indicators(bars: list[dict[str, Any]], indicators: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        ...
        for indicator in indicators:
            ...
            results.append({"name": name, "window": window, "data": result})
    ...
    def _series_to_points(timestamps: pd.Series, values: pd.Series) -> list[dict]:
        ...
        for i in range(len(timestamps)):
            if pd.notna(values.iloc[i]):
                points.append({"timestamp": int(timestamps.iloc[i]), "value": round(float(values.iloc[i]), 6)})
        return points
