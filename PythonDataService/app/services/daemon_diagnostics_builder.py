"""Pure daemon diagnostic report builder."""
...
from app.schemas.broker_session import BrokerSessionMirrorSnapshot, BrokerSessionRosterRow
...
from app.services.daemon_diagnostics_authoring import (
    ...
    _dominant_headline,
    _evidence,
    _first_matching_check,
    _headline_for,
    _host_restart_guidance,
    _instance_skip,
    _redact_facts,
    _row_evidence,
    _short_id,
...
def build_daemon_diagnostic_report(
    *,
    ...
    run_dir_visibility: Mapping[str, bool] | None = None,
) -> DaemonDiagnosticReport:
...
def build_run_dir_visibility(
    live_runs_root: Path,
    instances: HostRunnerInstancesStatus,
) -> dict[str, bool]:
    root = live_runs_root.resolve(strict=False)
    return {
        instance.run_id: _is_visible_run_dir(root, instance)
        for instance in instances.instances
    }


def _is_visible_run_dir(root: Path, instance: HostRunnerInstance) -> bool:
    try:
        candidate = (root / instance.run_id).resolve(strict=False)
...
def _lease_check(
    ...
    *,
    ...
    threshold_ms = health.lease_threshold_ms
    if threshold_ms is None:
        threshold_ms = _DEFAULT_LEASE_THRESHOLD_MS
...
def _instance_report(
    *,
    ...
    instances: HostRunnerInstancesStatus | None,
    mirror: BrokerSessionMirrorSnapshot,
    run_dir_visibility: Mapping[str, bool] | None,
) -> DaemonInstanceDiagnostic:
    ...
    checks.append(_artifacts_check(strategy_instance_id, registry, run_dir_visibility))
...
def _artifacts_check(
    ...
    if registry is None:
        return _instance_skip(
            ...
            "Run directory visibility waits for a managed run",
            strategy_instance_id,
            category=DaemonDiagnosticCategory.ARTIFACTS,
        )
    facts = {
        "run_id": registry.run_id,
        "run_dir": registry.run_dir,
        "data_plane_visible": (
            run_dir_visibility.get(registry.run_id)
            if run_dir_visibility is not None
            else None
        ),
    }
    if run_dir_visibility is None or registry.run_id not in run_dir_visibility:
        return _instance_skip(
            "instance.run_directory",
            "Run directory visibility could not be checked",
            strategy_instance_id,
            category=DaemonDiagnosticCategory.ARTIFACTS,
            summary="The daemon reported a run binding, but the data plane did not evaluate local artifact visibility.",
        )
    if not run_dir_visibility[registry.run_id]:
        return _authored(
            "instance.run_directory",
            DaemonDiagnosticCategory.ARTIFACTS,
            "fail",
            "Bot run directory is not visible",
            "The daemon has a live run binding, but the data plane cannot see the run directory under its configured live_runs_root.",
            remediation="Fix the live_runs_root configuration or artifact volume mount so the data plane and host daemon see the same run directory.",
            scope_ref=strategy_instance_id,
            evidence=_evidence(facts, redacted=True),
            condition=DaemonDominantCondition.RUN_DIR_INVISIBLE,
        )
    return _authored(
        "instance.run_directory",
        DaemonDiagnosticCategory.ARTIFACTS,
        "pass",
        "Bot run directory is visible",
        "The data plane can see the daemon-bound run directory.",
        scope_ref=strategy_instance_id,
        evidence=_evidence(facts, redacted=True),
    )
...
def _process_check(
    ...
    if registry is None:
        return _authored(
            ...
            remediation="Start this bot from its Broker V2 panel when you are ready to run it.",
            scope_ref=strategy_instance_id,
    ...
    if process.state == "exited":
        ...
        return _authored(
            ...
            remediation="Review the Broker V2 evidence before restarting this strategy instance.",
            scope_ref=strategy_instance_id,
            evidence=_evidence(facts),
