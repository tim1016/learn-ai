...
def lifecycle_record(strategy_instance_id: str) -> BotLifecycleStateRecord | None:
    ...
    """
    path = stable_bot_lifecycle_state_path(market_data_artifacts_root(), strategy_instance_id)
    try:
...
def terminal_duty_outcome(
    ...
    *,
    ...
    if latest is None or latest.state == "ACTIVE":
        return None
    try:
        receipt = live_state_binding_repository(market_data_artifacts_root()).read_outcome(
