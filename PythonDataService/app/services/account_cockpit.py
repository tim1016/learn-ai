    ...
    async def surface(self, *, account_id: str) -> AccountCockpitResponse:
        ...
        if clerk.journal.integrity == "corrupt":
            return AccountCockpitResponse(
                ...
                daemon=daemon,
                blockers=[
                    _journal_corrupt_blocker(
                        clerk.journal.corruption_detail,
                        clerk.journal.recovery_phase,
                    ),
                    *_unavailable_daemon_blockers(daemon),
                ],
            )
        if clerk.journal.integrity == "broker_evidence_only":
            return AccountCockpitResponse(
                ...
                daemon=daemon,
                blockers=[
                    _broker_evidence_hold_blocker(clerk.journal.corruption_detail),
                    *_unavailable_daemon_blockers(daemon),
                ],
            )
        if daemon.availability == "DOWN":
            return AccountCockpitResponse(
                ...
                clerk=clerk,
                daemon=daemon,
                blockers=_unavailable_daemon_blockers(daemon),
            )
...
def _unavailable_daemon_blockers(daemon: AccountCockpitDaemon) -> list[OperatorBlocker]:
    """Keep a host outage visible without displacing an account-safety ceremony."""

    if daemon.availability == "AVAILABLE":
        return []
    return [_daemon_down_blocker(daemon.reason_code, daemon.detail)]
