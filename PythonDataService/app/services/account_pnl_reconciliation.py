...
``local_delta = C2.realized_pnl_total + C2.open_pnl_total`` when C2 has
complete marks; and
...
marks do not align with C1's last equity timestamp, C3 reports
...
def reconcile_broker_curve_to_local_pnl(
    ...
    *,
    ...
    (realized_pnl_total + open_pnl_total)`` when all open lots are marked.
    ...
    A non-flat, unmarked FIFO book, or a mark from a different instant than
    C1's final equity point, is not a proof of the equity-curve delta. It is
    classified ``FIXTURE_INSUFFICIENT``; a numerical mismatch otherwise uses
    the taxonomy's ``PNL_DRIFT`` category.
    """
    ...
    if not local_attribution.marks_complete:
        divergences.append(
            AccountPnlDivergence(
                category=DivergenceCategory.FIXTURE_INSUFFICIENT,
                detail=(
    ...
    elif not _marks_align_to_curve_end(
        local_attribution,
        ...
        divergences.append(
            AccountPnlDivergence(
                category=DivergenceCategory.FIXTURE_INSUFFICIENT,
                detail=(
                    "Local FIFO open-lot marks do not align with the broker curve's "
                    f"final timestamp {broker_history.timestamps[-1]}."
                ),
            )
        )
...
def _local_delta(local_attribution: AccountPnlAttribution) -> float:
    """Return complete C2 P&L when marked, otherwise its diagnostic realized subtotal."""
    if local_attribution.marks_complete and local_attribution.open_pnl_total is not None:
        return local_attribution.realized_pnl_total + local_attribution.open_pnl_total
    return local_attribution.realized_pnl_total


def _marks_align_to_curve_end(
    local_attribution: AccountPnlAttribution,
    *,
    curve_end_ms: int,
) -> bool:
    """Require exact source-time alignment for every open-lot mark in C3."""
    return all(
