...
- Raw backend identifiers in Frontend receipt/evidence UI (`reason_code`, `gate_id`, `source`, receipt labels/values such as `NO_LIVE_BINDING` or `broker.connection`) must render through the shared `receiptLabel` pipe. Do not pipe backend-authored trader/operator prose; that copy should arrive from the backend or from a closed operator-copy map.
