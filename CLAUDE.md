...
- **Run project-scope lint before pushing; run targeted tests locally — the full test suite is CI's job.** The pre-commit hook (`lint-staged`) only checks the staged paths in *one* commit; cross-file drift (unused imports left after a refactor, sort order, ESLint warnings introduced indirectly) slips through. Lint is fast, so run the same lint command CI uses for whatever stack you touched: `ruff check PythonDataService/app/ PythonDataService/tests/`, `npx eslint Frontend/src/ --max-warnings 0`, or `dotnet format podman.sln --verify-no-changes`. Tests are not: locally run the suites for the surfaces your diff touches (and for whatever consumes a shared helper you edited), and let CI run the full per-stack sweep — see `.claude/rules/testing.md` (targeted-test policy, decided 2026-08-30, + how to baseline against inherited failures).
- **Run the `thermo-nuclear-code-quality-review` skill before pushing any PR, and address every major finding before push.** Lint and tests catch syntax and regressions; the thermonuclear review catches structural code-quality regressions — spaghetti branching, missed "code judo" simplifications, abstraction leakage, boundary leaks, canonical-helper duplication — that lint and tests cannot see. It is the gate between "the diff compiles" and "the diff is safe to ship". The skill (`~/.claude/skills/thermo-nuclear-code-quality-review/SKILL.md`) is model-invocable — **I invoke it myself via the Skill tool before first push** and address every major finding before push. No human gate. **Triage by severity:** every **major** finding (structural regression, spaghetti branching, missed code-judo, abstraction-boundary leak, canonical-helper duplication, hacky/magical abstraction) is a blocker — fix it in-branch before push, or, if explicitly accepted, document the reason in the PR description with the user's sign-off. **Minor** findings (cosmetic nits, low-conviction style notes, micro-suggestions) are optional — fix if cheap, defer if not; do not let them block the push or churn the diff. **File size is never a finding at any severity** (owner decision, 2026-09-15): do not report it, and never propose an extraction whose only justification is a line count. Size was only ever a proxy, and everything it proxied for — spaghetti branching, abstraction-boundary leaks, canonical-helper duplication, hacky abstractions — is named above and caught on its own merits. An extraction still earns its place when it removes duplication or fixes a boundary leak. The rule applies **only on the first push that opens the PR**. Re-pushes that address PR review comments (CodeRabbit, human reviewer, CI failures) do **NOT** re-trigger thermo — the gate is one-shot per PR. Re-pushes still require project-scope lint + the relevant test surface to be green.
