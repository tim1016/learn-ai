...
6. **Timestamps are `int64 ms UTC` everywhere.** Wire, storage, and in-memory. ISO strings and `DateTime` are disallowed as wire/storage formats. See `.claude/rules/numerical-rigor.md` → "Timestamp rigor" for the full policy and ban list.
