  ...
  it('seeds the form from ?search=, then opens a launched study by its job id', async () => {
    ...
    const view = await render(WalkForwardStudyPageComponent, {
      providers: [
        ...
        { provide: WalkForwardStudyService, useValue: { list, get: vi.fn(async () => study), preflight: vi.fn() } },
      ],
    });
    ...
    // The launched study opens in the History tab: its result view (not the history table) is rendered.
    await waitFor(() => expect(screen.getByRole('button', { name: /^history$/i })).not.toBeNull());
