 ...
 * one mounted: its own refresh/archive/cohort-flatten commands and their busy
 * state, so the header can render them without owning the roster's fleet read
 * itself.
 */
export interface BotsPageActionsHost {
  ...
  readonly openCohortFlatten: () => void;
}

/**
 * The seam between the account workspace header's action slot and the Bots
 * tab, whichever account is routed. The header renders Refresh, Archive
 * finished and Flatten cohort only while a host is registered — every other tab leaves the slot
