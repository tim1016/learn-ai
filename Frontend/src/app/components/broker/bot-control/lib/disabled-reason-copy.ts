...
// the *resolver* sorts the codes into; Bot Control's job is to take
 ...
 *  Bot Control can encounter on ``ActionCapability.disabled_reasons``
...
const OPERATOR_REASON_COPY: Record<OperatorReasonCode, string> = {
  // Action-conflict matrix. Bot Control does not ship a Reconcile
  ...
  STOPPED_REQUIRES_REDEPLOY:
    'Bot is STOPPED. Use Resume to clear the stop latch before starting; Pause is a no-op while stopped.',
...
const LOCAL_REASON_COPY: Record<LocalReasonCode, string> = {
  LOCAL_TRANSPORT_STALE:
    'Bot Control control-plane transport is not currently CONNECTED — refusing local dispatch until it recovers.',
