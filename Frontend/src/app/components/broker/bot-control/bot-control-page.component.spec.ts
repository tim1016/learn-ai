...
describe('BotControlPageComponent', () => {
  ...
  it('dispatchReconcileNow sets a human-readable mutationError when the reconcile call rejects', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const reconcileInstance = vi.fn().mockRejectedValue(new Error('Network error'));
    TestBed.configureTestingModule({
      providers: [
        provideZonelessChangeDetection(),
        provideRouter([]),
    ...
    expect(fixture.componentInstance.mutationError()).toContain('Network error');
    expect(fixture.componentInstance.busyAction()).toBeNull();
  });

  it('dispatchReconcileNow clears busyAction even when reconcile succeeds', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const reconcileInstance = vi.fn().mockResolvedValue({});
    TestBed.configureTestingModule({
      providers: [
  ...
  it('dispatchTraderGuidanceAction with focus_action selects the corresponding tab', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    TestBed.configureTestingModule({
      providers: [
        provideZonelessChangeDetection(),
    ...
    expect(fixture.componentInstance.selectedTab()).toBe('activity');
  });

  it('dispatchTraderGuidanceAction with none kind is a no-op and does not set an error', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    TestBed.configureTestingModule({
      providers: [
        provideZonelessChangeDetection(),
