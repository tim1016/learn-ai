...
const START_DISABLED_COPY: Record<HostProcessStartDisabledReasonCode, string> = {
  ...
  ACCOUNT_EVIDENCE_STALE:
    'The account check is overdue. Reconcile the connected broker account before starting this bot.',
 ...
 *
 * Renders the Bot Control surface that replaced the legacy
 * ``<app-broker-start-stop-card>`` (removed in the 2026-06-22 Bot Control
...
export class HostProcessNoticeComponent {
   ...
   * Bot Control surfaces what the host runner will do on its next start. */
