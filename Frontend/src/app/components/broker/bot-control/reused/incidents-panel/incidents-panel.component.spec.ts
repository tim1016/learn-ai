...
describe('IncidentsPanelComponent', () => {
  ...
  it('renders a source badge per row driven by incident_source', async () => {
    // PR-2 Bot Control work: every row shows a small badge (BROKER / APP /
  ...
  it('filters rows by source when a chip is clicked', async () => {
    // Bot Control filter is the per-session affordance from D7. Clicking
  ...
  it('renders viewer-local time as the primary timestamp and keeps raw_ts as a secondary mono', async () => {
    ...
    // UTC and ``ts_ms`` is canonical int64 ms UTC. Bot Control operator
