...
describe("AccountRosterPageComponent", () => {
  it("renders backend-owned roster facts in local time and opens the selected account desk", async () => {
    ...
    fireEvent.click(screen.getByText("DU1234567"));
  ...
  it("renders the configured-empty state", async () => {
    ...
    expect(
      screen.getByText("No configured accounts are available."),
    ).toBeTruthy();
  });
  ...
  it("renders a retryable initial error", async () => {
    ...
    failed.rosterErrorMessage.set(
      "Account roster is unavailable. Retry to request it again.",
    );
    await setup(failed);
  ...
  it("renders a stale last-good warning separately", async () => {
    ...
    expect(
      screen.getByText(
        "Showing the last available account roster. Refresh is unavailable.",
      ),
    ).toBeTruthy();
  });
});
