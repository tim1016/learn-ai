...
const QUICK_PROCEDURES: readonly QuickProcedure[] = [
  {
    ...
    steps: [
      ...
      'Verify On duty and confirm the selected effective posture. Paper orders are the deploy default; read-only remains available.',
    ],
    route: '/broker/bots',
  ...
  },
  {
    ...
    summary: 'Deploy together, then start with a fresh offer and verify each bot.',
    icon: 'pi pi-users',
    tone: 'scale',
    steps: [
      ...
      'Start each Ready bot only from its own fresh offer; every start is independently admitted.',
      'Do not activate the same bot three times within five minutes. After each start, verify the account remains unfrozen.',
    ],
    route: '/broker/bots',
    routeLabel: 'Open bots',
