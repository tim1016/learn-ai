...
describe('AccountDeskTraderEventsComponent', () => {
  it('renders only backend narration with local timestamp display', async () => {
    ...
    expect(screen.getByRole('listitem').textContent).toContain('The backend wrote this trader narration.');
    expect(screen.queryByText('Operator detail stays out of this feed.')).toBeNull();
