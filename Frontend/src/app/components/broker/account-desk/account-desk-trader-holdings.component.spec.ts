...
function baseStore() {
  ...
  return {
    ...
    account: signal<ReturnType<typeof makeAccountSummary> | null>(makeAccountSummary()),
    headlineMetrics: signal({ equity: 10_000, cash: 1_000, buyingPower: 20_000, dayPnl: 25, openPositions: 1 }),
...
describe('AccountDeskTraderHoldingsComponent', () => {
  ...
  it('follows the backend-authored warning move without constructing a cure from the owner class', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Open IBKR setup guide' }));

    expect(router.navigate).toHaveBeenCalledWith(['/docs/ibkr-setup-guide'], { fragment: undefined });
  });
  ...
  it('renders an explicit unavailable state rather than foreign-account holdings', async () => {
    const store = makeStore({
      ...
      unavailableMessage: signal('The connected broker session is attached to a different account. Live holdings are unavailable.'),
    });
    await setup(store);
