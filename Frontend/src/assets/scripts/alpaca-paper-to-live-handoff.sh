...
# Normalize one `compose port` answer into the address a host-side curl
# connects to. A wildcard binding (`0.0.0.0:8100`, the common default when
# no host IP is pinned) still answers on loopback; an explicit host is used
# verbatim.
published_host_address() {
  local published="$1"
...
# - Fleet: the coordinator mounts no configuration router of its own
...
# uses, so no service name is ever guessed here. The coordinator's
# *published* host port is likewise read from the deployment rather than
# assumed: the production posture publishes 8000-in-container as
# 127.0.0.1:${FLEET_COORDINATOR_PORT:-8100} (compose.fleet.yaml), so the
# combined posture's 127.0.0.1:8000 default must not survive onto fleet
# postures. FLEET_COORDINATOR_URL and FLEET_CLERK_ID exist only on
# enrolled lanes; their absence selects the combined posture's
# worker-direct behavior. All four exec/port probes are guarded because a
# failed probe exits nonzero, and `set -euo pipefail` must not read that
# as a failure of the ceremony itself.
...
if [[ -n "$coordinator_url" ]]; then
  ...
  published_port_line="$( ( "${compose_words[@]}" port "$coordinator_service" 8000 2>/dev/null || true) | tr -d '\r\n' )"
  if [[ -z "$published_port_line" ]]; then
    echo "The coordinator's published host port could not be resolved for $coordinator_service. Is the coordinator running, and does it publish its data-plane port?" >&2
    exit 1
  fi
  data_plane_url="http://$(published_host_address "$published_port_line")"
else
