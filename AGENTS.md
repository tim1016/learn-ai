...
- Symbol selection in Frontend UI must go through the shared instrument card (`app-instrument-card` or its picker wrappers) on the joined Alpaca+lake universe, per [ADR 0066](docs/architecture/adrs/0066-symbol-picker-offers-the-listing-universe-and-populates-the-lake.md). Never add a free-text ticker input, a hand-rolled suggestion list, or a label-map-as-membership — `TICKER_LABELS` is display metadata, not membership. An unheld pick is gated on its lake backfill inside the card (`EnsureCoverageService`); hosts never implement their own backfill gating or fallback symbol lists.
