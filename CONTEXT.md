...
- **Drain** — the host ceremony moving a clerk `provisioned -> draining`, recording `draining_since_ms`. It closes the lane's door (a draining clerk accepts no routed operation) and is deliberately ungated on entry, because the transition *is* the stop-accepting-new-work step. It is irreversible and idempotent: re-draining returns the existing record with its original start instant, never an extended deadline. Draining never asks whether the account is flat — open positions stay with the account, and moving infrastructure never forces a trading decision.
- **In-flight attempt** — a routing receipt with `state = 'not_dispatched'` and a non-null `dispatched_at_ms`: the coordinator handed the command to the lane and has not recorded an outcome. The same state with a null dispatch instant is the opposite fact, provably never sent. This is the registry's obligation ledger and the only in-flight evidence a drain may read.
- **Coordinator quiet** — no in-flight attempt exists for the clerk. Observed by the registry from its own table, inside the transaction that writes the lifecycle move. It is a real fact and a *partial* one: it proves the coordinator has no outstanding command, never that the lane has no outstanding work.
- **Lane quiet** — the lane holds no working order and runs no bot decision loop. Only the lane can observe it, and it reaches the registry as a **confirmation** fenced by the current session's instance and epoch — never as a heartbeat observation, which under the fleet's own rule confirms nothing. Its absence is recorded as absent and read as absent, never rounded up to a pass.
