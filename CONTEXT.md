...
- **Retry eligibility** — the earliest time session and retry-age policy permit an automatic reduction attempt. A quote, custody proof and a recovery pass are still required; eligibility is not a scheduled submission or proof that a sell failed.
...
- **Staged selection** — the profile revision selected for a possible future Apply; it governs no running bot.
- **Effective revision** — the profile revision acknowledged by the running worker after an explicit Apply and controlled restart. A merely staged revision cannot replace it during crash recovery.
- **Apply** — the operator's one-shot request to make an exact staged revision effective at the next controlled restart. It grants no live arming permission.
- **Credential slot** — an opaque name for a permitted broker credential pair supplied to an installation. It contains no secret and grants no account access by itself.
...
- **Worker key** — the durable per-clerk worker identity ADR 0060 deferred and ADR 0062 designs. It never crosses the public API and (ADR 0062 addendum, 2026-09-13) never authenticates a transport; each direction of internal traffic instead carries its own environment-only **service token**, minted by the host ceremony and stored nowhere.
- **Routing epoch** — the monotonic identity of one agent registration; retries must not silently cross an epoch change.
- **Effective binding generation** — the clerk-local counter that changes only when the clerk's effective profile, revision or account changes; commands carry the generation they were prepared against. The coordinator's **confirmed binding observation** on the assignment row is the exact generation a confirming worker presented, fenced by its instance and epoch — the routing fence for execution operations. Heartbeat-reported binding facts are observations only and confirm nothing.
- **Account assignment** — the fleet reservation for `(broker, canonical external account id)`. Only one reserved or effective assignment exists per key, it never expires into takeover (heartbeat loss marks the lane unreachable but transfers nothing), and reassignment is a proof-driven host ceremony. Re-reserving an account the owner already holds is the same-owner resume and returns the row unchanged.
...
- **Drain** — the host ceremony moving a clerk `provisioned -> draining`, recording `draining_since_ms`. It closes the lane's door (a draining clerk accepts no routed operation) and is deliberately ungated on entry, because the transition *is* the stop-accepting-new-work step. It is irreversible and idempotent: re-draining returns the existing record with its original start instant, never an extended deadline. A clerk that has ever served must pass through it to reach `retired`; only a provably never-served clerk may take the direct edge. **"Never served" is a three-clause predicate**: no row in the append-only assignment history, no row in the append-only session history, **and no live `clerk_sessions` row** — the current-session table is an upsert keyed on `clerk_id` and is never deleted, so a clerk that registered exactly once holds a live row with no history row at all, and the two history clauses alone would misclassify it. In practice the predicate means **never booted**: a lane whose agent ever registered is a served clerk for life, while a provisioned lane that never started stays directly retirable. Draining never asks whether the account is flat: open positions stay with the account, and moving infrastructure never forces a trading decision.
- **Unsettled attempt** — a routing receipt with `state = 'not_dispatched'` and a non-null `dispatched_at_ms`: the coordinator dispatched a command and *lost its outcome*. The same state with a null dispatch instant is the opposite fact, provably never sent.
- **Command quiet** — no unsettled attempt exists for the clerk. A real but narrow fact: it proves the coordinator lost no outcome, and nothing whatever about orders. A receipt settles `delivered` the instant the lane returns a sub-400 response, so a market order accepted and still working at the broker leaves the ledger quiet.
- **Order quiet** — no order placed by or through the lane is still working at the broker. **Not observable by the fleet layer, by construction**: receipts settle on the HTTP response, the 76-operation routed catalog is a bot and configuration control surface so bot-runner orders never reach the coordinator, and routed SSE streams open no receipt at all. Only the lane, which holds the broker connection and its own custody database, can answer it.
- **Lane quiet** — the lane's own assertion, about itself, that five conditions hold together at one observed instant: **draining** (it accepts no new routed work and starts no new bots), **no bot running** (the runner confirmed its exit — not the panel's desired state `STOPPED`, which records only what the operator wants), **every working order on the account ended** (a broker-reported finished state — "cancel sent" is not an ending, because a cancel races a fill; note this is *broader* than **order quiet** above, which is lane-scoped: lane quiet reads the provider's own order list for the whole account, because an order the lane did not place can still fill and re-open a position, and an account is bound to one lane at a time so the account answer is the lane answer), **flat** (zero open positions across the whole account, from the provider's own position list rather than bot-attributed custody), and **no order intent in flight** (every durable intent has a resolved broker outcome — `unresolved_intent_refs` is a separate set from `working_order_refs`, and an intent with no established broker identity is outstanding, not absent). Owner decision 2026-09-19, amending ADR 0063 Decisions 2 and 5. The operator performs the acts with panel actions that already exist (`stop`, `cancel_verified_working_orders`, `flatten_stop`, in that order, waiting for the flatten's own orders to end) and **the lane proves the result**, because a click is not evidence; a position or order opened by hand survives every panel action, so the lane names it and the operator closes it at the broker. It reaches the registry as a **confirmation** fenced by the current session's instance and epoch and refused before `draining_since_ms` — never as a heartbeat observation, which under the fleet's own rule confirms nothing. Silence and staleness make the gate **refuse**, never fire, which is the opposite direction to the staleness ADR 0063 Decision 5 forbids; the staleness measured is the confirmation's own `observed_at_ms`, never session liveness, so a live lane that stopped confirming refuses exactly as a dead one does. The Alpaca lane answers it on every heartbeat while draining (#2154, 2026-09-22), re-reading the account twice because its order and position lists carry no consistency fence; a missing, stale or unquiet answer is recorded as such, never rounded up to a pass. A lane that has answered quiet retires on the normal path, so the auditable count of forced retirements falls below 100% — but not to zero — a lane that cannot answer, or whose operator declines to discharge an outstanding obligation, still exits through `force-retire`, there preceded by cancelling the orders and closing the positions directly at the broker.
- **Mount attestation** — the one operator input that survives the drain ceremony, asserting the old volume is offline. It survives because the host alone can observe its own mounts and the coordinator is deliberately denied the container authority that would let it look. Recorded as a bounded attributable `operator` plus `change_ref`, which buys attribution rather than proof. It covers only the volume claim: of the retired token's four bundled claims, the provider's-obligations claim is lane quiet's intended proof once a provider can answer it, and the old-agent-offline and credentials-isolated claims are unhomed today — named as such rather than dropped. It replaces a fixed release-proof phrase that was published in the repository that checked it. It is consumed by release and by reassignment — one attestation per ceremony invocation, not one per account or clerk — and each additionally waits out the same deadline that bounds `force-retire` before proceeding without lane quiet (ADR 0063 §4.1). Drain entry consumes no attestation and is ungated. **Reassignment is unreachable today** as the joint consequence of that draining-predecessor precondition and the rule that no drained lane may be reassigned while a restart during a coordinator outage can resurrect its binding; it unblocks only when that indistinguishability is closed. Release is unaffected, so drain → wait → release → `force-retire` remains a complete exit from the fleet — and blocking reassignment is a fact about one ceremony's reachability, not a safety property, because release followed by a fresh reservation of the released key reaches the same two-writer handover in two steps with less evidence.

## Data lake (resolved 2026-08-27)

**Lineage: live.**

- **Data lake** — the authority for historical bar data (ADR 0049): immutable LEAN-format files hold the canonical bytes, with Postgres as a catalog and coordination plane over them. Replaces the policy-keyed cache, which had bytes but no catalog. Governs *historical* bars only; the live broker feed is a separate concern and is not a market-data source.
- **Lake catalog** — the Postgres side of the lake: rows describing which artifacts exist, their hashes, provenance, and outstanding claims. It holds statements *about* market data, never market data. Its index (identity, hash, coverage) is rebuildable by walking the lake and re-hashing; lease state is deliberately not rebuilt (a rebuilt lease would be a stale claim, not a repaired one). Provenance is a Postgres-embedded projection of a file-side original that today lives in the legacy policy tree, not the lake itself — ADR 0049 records an explicit obligation on the retirement slice to migrate that file-side home into the lake before the policy tree is deleted, so the audit trail never depends on Postgres alone. Unrelated to the **strategy catalog** of the Strategy Validation surface.
