...
- **Lane quiet** — the lane's assertion of order quiet plus no admitting bot runner, about itself. It reaches the registry as a **confirmation** fenced by the current session's instance and epoch — never as a heartbeat observation, which under the fleet's own rule confirms nothing. No provider can answer it yet, so the normal retirement path is blocked rather than degraded; its absence is recorded as absent, never rounded up to a pass.
- **Mount attestation** — the one operator input that survives the drain ceremony, asserting the old volume is offline. It survives because the host alone can observe its own mounts and the coordinator is deliberately denied the container authority that would let it look. Recorded as a bounded attributable `operator` plus `change_ref`, which buys attribution rather than proof. It covers only the volume claim: of the retired token's four bundled claims, the provider's-obligations claim is lane quiet's intended proof once a provider can answer it, and the old-agent-offline and credentials-isolated claims are unhomed today — named as such rather than dropped. It replaces a fixed release-proof phrase that was published in the repository that checked it. Release and reassignment, which also consume this attestation, additionally wait out the same deadline that bounds `force-retire` before they proceed without lane quiet (ADR 0063 §4.1).
