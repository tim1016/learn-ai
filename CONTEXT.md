...
- **Lake catalog** — the Postgres side of the lake: rows describing which artifacts exist, their hashes, provenance, and outstanding claims. It holds statements *about* market data, never market data. Its index (identity, hash, coverage) is rebuildable by walking the lake and re-hashing; provenance keeps a file-side home so the audit trail stays on files per ADR 0001; lease state is deliberately not rebuilt. Unrelated to the **strategy catalog** of the Strategy Validation surface.
