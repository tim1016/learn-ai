...
- **Clerk-scoped surface** — the coordinator's public routes under `/api/brokers/{broker}/clerks/{clerk_id}`, one per catalog operation plus the directory. The coordinator resolves, fences and forwards; it never serves an unscoped agent family itself, and a clerk agent mounts none of the routing surface.
...
- **Composed auth** — the browser secret terminates at the coordinator: public routes demand the data-plane control secret, while an internal forward presents the per-clerk coordinator service token, accepted at the same guard. Cleartext internal traffic never leaves the private network boundary — an `http://` destination naming a public address is refused before any byte is sent.
