...
- **Lane quiet** — the lane's own assertion, about itself, that four conditions hold together at one observed instant: **draining** (it accepts no new routed work and starts no new bots), **no bot running** (the runner confirmed its exit — not the panel's desired state `STOPPED`, which records only what the operator wants), **order quiet** (every working order has reached a broker-reported finished state, a resting GTC included — "cancel sent" is not an ending, because a cancel races a fill), and **flat** (zero open positions across the whole account, read from the provider's own position list rather than from bot-attributed custody, so a hand-opened leg is seen and named rather than silently passed). Owner decision 2026-09-19, amending ADR 0063 Decisions 2 and 5. The operator performs the acts with panel actions that already exist (`stop`, `cancel_verified_working_orders`, `flatten_stop`, in that order, waiting for the flatten's own orders to end) and **the lane proves the result**, because a click is not evidence. It reaches the registry as a **confirmation** fenced by the current session's instance and epoch and refused before `draining_since_ms` — never as a heartbeat observation, which under the fleet's own rule confirms nothing; silence, staleness and unreachability make the gate **refuse**, never fire, which is the opposite direction to the staleness ADR 0063 Decision 5 forbids. No provider can answer it yet (#2154), so the normal retirement path stays blocked rather than degraded and its absence is recorded as absent, never rounded up to a pass: until it ships, **every** retirement of a served clerk is the separately named, deadline-conditioned, operator-attributed `force-retire`, and the auditable count of forced retirements is 100%. Once it ships that count falls but not to zero — a lane that cannot answer, or whose operator declines to discharge an outstanding obligation, still exits through `force-retire`, there preceded by cancelling the orders and closing the positions directly at the broker.
