...
- **Lane quiet** — the lane's assertion of order quiet plus no admitting bot runner, about itself. It reaches the registry as a **confirmation** fenced by the current session's instance and epoch — never as a heartbeat observation, which under the fleet's own rule confirms nothing. No provider can answer it yet, so the normal retirement path is blocked rather than degraded; its absence is recorded as absent, never rounded up to a pass. The consequence is not a rare exception: until a provider ships it, **every** retirement of a served clerk is the separately named, deadline-conditioned, operator-attributed `force-retire`, and the auditable count of forced retirements starts at 100% and stays there. One case outlives that fix — a lane holding a resting GTC order can never reach lane quiet, so it is permanently force-retire-only; lane quiet is not scoped around it, because a resting GTC is a working order and excusing it would weaken the only gate that speaks to the broker.
